...
{
	$it_name = explode("||", $oneday_item['it_name']);
	$it_name = $it_name[0] . $it_name[1];
	$l_it_name = explode("||", $oneday_item['l_it_name']);
	$l_it_name = $l_it_name[0] . $l_it_name[1];
	$list_li .= "
        <li>
            <div class='no'>
                <div class='number'>".++$ii.$oneday_item['sort']."</div>
                <input type='hidden' name='sort[".$oneday_item['uid']."]' value='".$oneday_item['sort']."' class='sort'>
            </div>
            <div class='it_img'>".get_it_image($oneday_item['l_it_id'].'_s',100,100,$oneday_item['it_id'])."</div>
            <div class='it_info'>
                <p>
                    상품코드 : <a href='".$g4['shop_path']."/item.php?it_id=".$oneday_item['it_id']."' target='_blank'>".$oneday_item['it_id']."</a> / 
                    참고상품코드 : <a href='".$g4['shop_path']."/item.php?it_id=".$oneday_item['l_it_id']."' target='_blank'>".$oneday_item['l_it_id']."</a>
                </p>
                <p>상품명 : <span title='".$oneday_item['it_name']."'>".$it_name."</span></p>
                <p>참고 상품명 : <span title='".$oneday_item['l_it_name']."'>".$l_it_name."</span></p>
                <p>총재고(실/가) : ". $oneday_item['real_qty'] ." / ".( $oneday_item['real_qty'] * $oneday_item['multiplication'] )."</p>
                <p>잔여재고(실/가) : <b>".( $oneday_item['real_qty'] - $oneday_item['order_cnt'])."</b> / ".( ($oneday_item['real_qty'] * $oneday_item['multiplication']) - ($oneday_item['order_cnt'] * $oneday_item['multiplication']) )."</p>
                <p>
                    주문수량 : ".$oneday_item['order_cnt']." / 
                    주문금액 : ".number_format($oneday_item['total_amount'])." / 
                    이벤트일시 : ".$oneday_item['st_dt']." ~ ".$oneday_item['st_dt']." / 
                    판매금액 : ".number_format($oneday_item['price'])."
                </p>
            </div>
            <div class='list_btn'>".
