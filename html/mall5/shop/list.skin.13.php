...
if (count($arr) > 0)
{
    ...
    while (list($key, $row) = each ($arr))
    {
         ...
         if ($save_ca_id != $row[ca_id])
        {
            ...
            echo "
            <tr height=50px>
                <td style='border-bottom:2px solid #D4D4D4;padding-top:20px' colspan=5>&nbsp;<img src='$g4[shop_img_path]/icon3.gif'> <b><a href='./list.php?ca_id=$row[ca_id]'>$tmp_ca[ca_name]</a></b></td>
            </tr>";
        ...
        echo "<tr><td align=center>".it_name_icon($row,$row['it_name'],1,'list')."</td></tr>";
...
if (($cnt = $i%$list_mod) != 0)
    for ($k=$cnt; $k<$list_mod; $k++)
        echo "<td>&nbsp;</td>\n";
?>
<tr><td colspan='<?=$list_mod?>' style='border-bottom:1px solid #D4D4D4;padding-top:20px'></td></tr>
</table>
