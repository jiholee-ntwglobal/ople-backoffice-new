...
if(date('Ymd') >= '20141101' && date('Ymd') <= '20141103'){ // 7월 5일부터 7일까지 주문건만
	...
	$od_total_amount = $od_receipt_bank + $od_receipt_card + $od_receipt_point; // 총 주문액
	if($od_total_amount >= 60000){
		$od_shop_memo = "6만원 이상 구매 고객 증정 QUEEN HELENE Stick Cocoa Butter 1oz upc 79896049462\n6만원 이상 구매 고객 증정 QUEEN HELENE Stick Cocoa Butter 1oz upc 79896049462\n6만원 이상 구매 고객 증정 QUEEN HELENE Stick Cocoa Butter 1oz upc 79896049462\n";
	}
}

if(date('Ymd') >= '20141108' && date('Ymd') <= '20141110'){ // 7월 5일부터 7일까지 주문건만
	# 3만원 이상 주문시 관리자 메모 등록
	$od_total_amount = $od_receipt_bank + $od_receipt_card + $od_receipt_point; // 총 주문액
	if($od_total_amount >= 60000){
		$od_shop_memo = "6만원 이상 구매 고객 증정 Avalon Nourishing Organic Lip Balm Cherry Aloe 0.15 OZ \n6만원 이상 구매 고객 증정 Avalon Nourishing Organic Lip Balm Cherry Aloe 0.15 OZ \n6만원 이상 구매 고객 증정 Avalon Nourishing Organic Lip Balm Cherry Aloe 0.15 OZ \n";
	}
}
		...
		if(!$chk_ma['ma_pid']){
			$ma_insertQ = "
				...
				set	mb_id		= '{$member['mb_id']}',
					...
					ma_addr1	= '$od_b_addr1',
					ma_addr2	= '$od_b_addr2',
					ma_addr_jibeon = '$_POST[od_b_addr_jibeon]',
					ma_datetime	= '{$g4['time_ymdhis']}'
			";

			sql_query($ma_insertQ);
		}
	}
