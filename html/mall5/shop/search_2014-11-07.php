...
if($it_maker){
	...
	if(is_numeric ($search_str) ){ // 검색어가 숫자만 있다면 UPC or it_id로 검색 2014-10-17 홍민기
		...
		$search_str_escape = str_replace('|','',$search_str);
		$search_str_escape = str_replace('|','',$search_str);
		$search_str_escape = str_replace(' ','|',$search_str_escape);
		$sql_where .= ($sql_where ? " and ":"")."(".$search_where." or a.it_opt4 regexp '".str_replace(' ','|',$search_str)."') \n";
	}
	...
	$sql = "
		select
			...
			".($search_where ? ",(".$search_where." + a.it_opt4 regexp '".str_replace(' ','|',$search_str)."') as score":"")."
		from
			".$sql_from."
		where
			".$sql_where.$sql_where_search."
