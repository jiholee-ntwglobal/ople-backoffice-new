...
/* 보안 이슈로 인한 결제금액 재계산 2015-05-04 홍민기 */
if($_POST['od_temp_point'] > $member['mb_point']){ // 보유 포인트 검사
    alert("회원님의 포인트가 부족하여 포인트로 결제 할 수 없습니다.");
    exit;
}

$_POST['od_amount'] = sql_fetch("select sum(ct_qty*ct_amount) as od_amount from ".$g4['yc4_cart_table']." where on_uid = '".$tmp_on_uid."' and ct_status = '쇼핑' and ihappy_fg is null");
$_POST['od_amount'] = $_POST['od_amount']['od_amount']; // 상품금액

// 배송비 책정
if ($default['de_send_cost_case'] == "없음") {
    $_POST['od_send_cost'] = 0;
}elseif(manager_chk($member['mb_id'])){
    $_POST['od_send_cost'] = 0;
}else{
    if( $_BAESONG_CHANGE && point_pay_chk($tmp_on_uid) ){
        $send_cost_data = new_send_cost_chk($tmp_on_uid);
        $_POST['od_send_cost'] = $send_cost_data['send_cost'];
    }else{
        $_POST['od_send_cost'] = old_send_cost_chk($_POST['od_amount2']);
    }
}

$_POST['od_settle_amount'] = $_POST['od_amount'] + $_POST['od_send_cost']; // 총 결제금액

$od_temp_amount = $_POST['od_settle_amount'] - $_POST['od_temp_point']; // 실제 결제 금액(포인트 금액을 제외한 실 결제금액)
if($_POST['od_settle_case'] == '신용카드'){
    $_POST['od_receipt_card'] = $od_temp_amount;
}else{
    $_POST['od_receipt_bank'] = $od_temp_amount;
}
...
if(date('Ymd') >= '20141115' && date('Ymd') <= '20141117'){ // 7월 5일부터 7일까지 주문건만
	# 3만원 이상 주문시 관리자 메모 등록
	$od_total_amount = $od_receipt_bank + $od_receipt_card + $od_receipt_point; // 총 주문액
	if($od_total_amount >= 60000){
		$od_shop_memo = "6만원 이상 Fuchs Record V Adult Soft Natural Bristle Toothbrush 훅스 레코드 브이 성인 천연 소프트 칫솔 1개 (색상랜덤) UPC 07956500901 증정 \n6만원 이상 Fuchs Record V Adult Soft Natural Bristle Toothbrush 훅스 레코드 브이 성인 천연 소프트 칫솔 1개 (색상랜덤) UPC 07956500901 증정 \n6만원 이상 Fuchs Record V Adult Soft Natural Bristle Toothbrush 훅스 레코드 브이 성인 천연 소프트 칫솔 1개 (색상랜덤) UPC 07956500901 증정 \n";
	}
}
...
# +1 이벤트 상품이 존재한다면 insert
if(is_array($ov_cart_add_sql)){
	foreach($ov_cart_add_sql as $val){
		sql_query($val);
	}
}
if(is_array($cart_add_sql)){
	foreach($cart_add_sql as $val){
		sql_query($val);
	}
}
...
$sql = "update $g4[yc4_cart_table]
           set ct_status = '주문'
               $sql_card_point
         where on_uid = '$tmp_on_uid' and ihappy_fg is null ";
sql_query($sql);
...
// 오플 장바구니의 on_uid를 변경된 on_uid로 변경
sql_query("update ".$g4['yc4_cart_table']." set on_uid = '".get_session('ss_on_uid')."' where on_uid = '".$tmp_on_uid."' and ihappy_fg = 'y'");
