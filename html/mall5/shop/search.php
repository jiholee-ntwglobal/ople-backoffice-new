...
if($it_maker){
	...
	$sql_where .= ($sql_where ? " and ":"")."lower(trim(a.it_maker)) = '".strtolower($it_maker)."' \n";
    if(!$admin_chk){
        $sql_where .= ($sql_where ? " and ":"")." b.it_id is not null";
    }
	...
	if(is_numeric ($search_str) ){ // 검색어가 숫자만 있다면 UPC or it_id로 검색 2014-10-17 홍민기
		...
		$upc_chk = sql_query("
			select it_id from ople_mapping where upc = '".$search_str."'
		");
		$upc_search = '';
		while($row = sql_fetch_array($upc_chk)){
			$upc_search .= ($upc_search ? ",":"")."'".$row['it_id']."'";
		}
		if($upc_search){
			$upc_search = "a.it_id in(".$upc_search.") or a.it_id = '".$search_str."'";
		}else{
			$upc_search = "a.it_id = '".$search_str."'";
		}
		$sql_where .= ($sql_where ? " and ":"") . " (".$upc_search.")";
