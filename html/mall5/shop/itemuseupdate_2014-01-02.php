	...
	if($_POST['od_id']){ // 주문번호가 넘어왔을 경우
		...
		$od_cnt_chk = sql_fetch("
			...
			where
					it_id='$it_id'
				and ct_status = '완료'
				and ct_mb_id='{$member['mb_id']}'
		");

		// 후기 건수 (본 회원의 해당 상품 후기)
		$item_ps_cnt_chk = sql_fetch("
			select
				count(*) as cnt
			from
				".$g4['yc4_item_ps_table']."
			where
					mb_id = '".$member['mb_id']."'
				and it_id = '".$it_id."'
		...
		# 주문건수가 해당상품 건수보다 많을경우에만 od_id를 찾는다 2015-01-07 홍민기 #
		if($item_ps_cnt_chk['cnt'] <= $od_cnt_chk['cnt']){
			...
			$od_chk = sql_query("
				...
				where
					...
					and	a.it_id='$it_id'
					and a.ct_status = '완료'
					and a.ct_mb_id='{$member['mb_id']}'
				order by a.ct_time asc
				limit 1
			");
			while($row = sql_fetch_array($od_chk)){
				# 해당주문서로 이미 작성된 본 상품 후기가 있는지 체크 #
				$before_item_ps_cnt = sql_fetch("
					select count(*) as cnt from ".$g4['yc4_item_ps_table']." where it_id = '".$it_id."' and od_id = '".$row['od_id']."'
				");
				if($before_item_ps_cnt['cnt'] == 0){
					$od_id = $row['od_id'];
					break;
				}
			}
