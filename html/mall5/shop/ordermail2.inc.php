...
// 김선용 201207 : 메일발송주소 고정처리
if($default['de_post_mail_addr'] != '')
	$from_mail['mb_email'] = $default['de_post_mail_addr'];
else
	$from_mail = get_admin('super');

if($default['de_receive_mail_addr'] != '') // 관리자 주문메일 사본 수신주소
	$to_mail['mb_email'] = $default['de_receive_mail_addr'];
else
	$to_mail = get_admin('super');
//------------------------------------------------------------------------------
// 운영자에게 메일보내기
//------------------------------------------------------------------------------
$subject = "{$default[de_admin_company_name]}에서 주문이 들어 왔습니다. ($od_name)";
ob_start();
include "./mail/orderupdate1.mail.php";
$content = ob_get_contents();
ob_end_clean();

mailer($od_name, $od_email, $default['de_receive_mail_addr'], $subject, $content, 1);
//------------------------------------------------------------------------------
...
mailer($default['de_admin_company_name'], $default['de_post_mail_addr'], $od_email, $subject, $content, 1);
//------------------------------------------------------------------------------
...
for ($i=0; $row=sql_fetch_array($result); $i++)
{
    ...
    $list[it_name] = get_item_name($row[it_name]);
    ...
    mailer($default[de_admin_company_name], $default['de_post_mail_addr'],  $row['it_sell_email'], $subject, $content, 1);
}
//==============================================================================
?>