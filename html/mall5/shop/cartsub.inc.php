...
if($_SERVER['REMOTE_ADDR'] == '59.17.43.129'){
	$nonstop_data = sql_fetch("select uid,it_id,ev_amount from yc4_nontop_sale where status=2");

	$nonstop_event_item[$nonstop_data['it_id']] = $nonstop_data['ev_amount'];

	$nonstop_rs = sql_query("select it_id,ev_amount from yc4_nontop_sale where uid < $nonstop_data[uid]");
	while($nonstop_end_data = sql_fetch_array($nonstop_rs)){
		$nonstop_event_end_item[$nonstop_end_data['it_id']] = $nonstop_end_data['ev_amount'];
	}
}
...
for ($i=0; $row=sql_fetch_array($result); $i++)
{
	...
	if($nonstop_event_item[$row['it_id']] > 0){
		$row['ct_amount'] = $nonstop_event_item[$row['it_id']];
		sql_query("update $g4[yc4_cart_table] set ct_amount='$row[ct_amount]' where on_uid = '".$s_on_uid."' and it_id = '".$row['it_id']."'");
	}

	if($nonstop_event_end_item[$row['it_id']] > 0){
		$row['ct_amount'] = $row['it_amount'];
		sql_query("update $g4[yc4_cart_table] set ct_amount='$row[it_amount]' where on_uid = '".$s_on_uid."' and it_id = '".$row['it_id']."'");
	}
	...
	if($oneday_data){
		...
		if($s_page == 'orderform.php' && !$nonstop_event_item[$row['it_id']] ){
			$it_ori_amount = sql_fetch("select it_id,it_amount,it_tel_inq from ".$g4['yc4_item_table']." where it_id = '".$row['it_id']."'");
			$it_ori_amount = get_amount($it_ori_amount);
			if($row['ct_amount'] != $it_ori_amount){
				sql_query( "update ".$g4['yc4_cart_table']." set ct_amount = '".$it_ori_amount."' where ct_id = '".$row['ct_id']."'");
				$row['ct_amount'] = $it_ori_amount;
				$notice_msg = "판매가가 변경된 상품이 존재합니다.";
			}
...
//        $continue_ca_id = $row[ca_id];
    ...
    if ($s_page == "cart.php" || $s_page == "orderinquiryview.php") { // 링크를 붙이고
        $a1 = "<a href='http://www.ople.com/mall5/shop/item.php?it_id=$row[it_id]'>";
        $a2 = "</a>";
    ...
    // 수량, 입력(수량)
    if ($s_page == "cart.php")
        echo "<td><input type='number' min='1' id='ct_qty_{$i}' name='ct_qty[{$i}]' value='$row[ct_qty]' size=4 maxlength=6 class=ed style='text-align:right;padding-right:3px; width:44px;' autocomplete='off'></td>";
	else
	...
	<td>
		<span class='amount'>
			<span>￦</span>" . number_format($row[ct_amount]) . " <br />
			(<span>$</span>" . number_format( ($row['ct_amount_usd'] ? $row['ct_amount_usd'] :usd_convert($row['ct_amount'] )),2).")
		</span>
	</td>
	";
    echo "
	<td>
		<span class='amount'>
			<span>￦</span>" . number_format($sell_amount) . " <br />
			(<span>$</span>" . number_format( $row['ct_amount_usd'] ? $row['ct_amount_usd'] *$row['ct_qty'] : (usd_convert($row['ct_amount'] * $row['ct_qty'])),2).")
		</span>
	</td>
	";
...
if ($i == 0) {
    ...
    if ($_POST[od_send_cost]) {
        ...
        if ($default[de_send_cost_case] == "없음"){
            $send_cost = 0;
		}elseif(manager_chk($member['mb_id'])){
			$send_cost = 0;
		}
        else {
            if( $_BAESONG_CHANGE && point_pay_chk($s_on_uid) ){
				$send_cost_data = new_send_cost_chk($s_on_uid);
				$send_cost = $send_cost_data['send_cost'];
			}else{
		...
		// if ($row && $row[od_send_cost] > 0) 배송비 프로모션으로 인한 일시작 배송비 무료일 경우 예외처리 2015-02-24 홍민기
        if ($row)
		...
		if(in_array($member['mb_level'], array('3', '4'))){
            ...
            if($_SERVER['REMOTE_ADDR'] == '59.17.43.129'){
                foreach($post_arr as $val){
                    list($lv,$free_fg) = explode('=>',$val);
                    if($member['mb_level'] == $lv && $free_fg){
                        $send_cost = 0;
                        break;
                    }
                }
    ...
    if ($send_cost > 0)
    {
		...
		echo "<td colspan='4' rowspan='".$rowspan."' style='width:735px;'>".($ov_evt_flag  ? "※ +1 이벤트 사은품은 주문서 작성 완료 후 주문내역에 추가됩니다.":"")."</td>";
        echo "<td style='text-align:right;'>배송비</td>";
        echo "
		<td colspan=3 style='text-align:right;padding-right:10px;'>
			￦ " . number_format($send_cost) . " ($ ".number_format(usd_convert($send_cost),2).")
		</td>";
        // echo "<td>&nbsp;</td>";
        if ($s_page == "cart.php" || $s_page == "orderinquiryview.php")
        // echo "<td>&nbsp;</td>";
	...
	if($s_page == 'orderinquiryview.php' && $od['od_recommend_off_sale']) // 주문조회시
	{
		// echo "<tr><td colspan='$colspan' height=1 bgcolor=#E7E9E9></td></tr>";
		echo "<tr class='tatal_Area'>";
		echo "<td style='text-align:right;'><span style='color:blue;'>※ 추천인등록 시스템에 의한 할인적용</span></td>";
		echo "<td colspan=3 style='text-align:right;padding-right:10px;'>-".nf($od['od_recommend_off_sale'])."원</td>";
		// echo "<td>&nbsp;</td>";
		// echo "<td>&nbsp;</td>";
		echo "</tr>";
	}
	else if($recommend_off_sale && $s_page != 'orderinquiryview.php') // 주문시
	{
		...
		echo "<tr class='tatal_Area'>";
		echo "<td style='text-align:right;'><span style='color:blue;'>※ 추천인등록 시스템에 의한 할인적용</span> : </td>";
		echo "<td colspan=3 style='text-align:right;padding-right:10px;'>-".nf($default['de_recom_off_amount'])."원</td>";
		//echo "<td>&nbsp;</td>";
		if ($s_page == "cart.php")
		  // echo "<td>&nbsp;</td>";
		echo "  </tr>   ";
	...
	if($od['od_dc_amount']){
		echo "<tr class='tatal_Area'>";
		echo "<td colspan='".$colspan2."' style='text-align:right;'>할인금액</td>";
		echo "<td colspan='3' style='text-align:right;padding-right:10px;'>".number_format($od['od_dc_amount'])."</td>";	
		echo "</tr>";

		$tot_amount = $tot_sell_amount + $send_cost - $od['od_dc_amount'];
	}
    ...
    echo "
	<td colspan=3 style='text-align:right;padding-right:10px;'>
		<strong>￦ " . number_format($tot_amount) . "</strong>
		($ ".number_format(usd_convert($tot_amount, ($od['exchange_rate']) ? $od['exchange_rate'] : null),2).")
	</td>";
...
<?php
if($_BAESONG_CHANGE && $s_page == "cart.php"){
?>
	<!-- 20150320_무료배송 프로모션 시작 -->

	<div class="banner_Promotion">
		<p class="price_USD_cart"><strong> $ 40 </strong>(<?php echo number_format(40 * $default['de_conv_pay']);?>원)이상 구매 시 <em>(단, 건강기능식품 <?php echo $default['no_send_cost_health_cnt'];?>병까지 무료이며, 7병 이상은 기존 배송비 그대로 부여됩니다.)</em></p>
		<ul>
        	<li>건강기능식품 : <strong><?php echo number_format($send_cost_data['health_cnt']);?></strong></li>
			<li>총 구매금액 : ￦ <strong><?php echo number_format($send_cost_data['tot_amount']);?></strong></li>
			<?php if($send_cost_data['send_promo_fg']){?>
				<li>배송비 : <strong>무료</strong></li>
			<?php }else{?>
				<li>배송비 : ￦ <strong><?php echo number_format($send_cost_data['send_cost']);?></strong></li>
	<?php }?>
		</ul>
	</div>
	<!-- 20150320_무료배송 프로모션 끝 -->
	<br/>
<?php
}
?>
...
<div>
<a href='<?=$g4['shop_path']?>/persnoal_number_info.php' target='_blank'>
<img src='http://115.68.20.84/mall6/page/etc/unipass_B.jpg' />
</a>
</div>

<div style="margin-top:10px;">
<img src="http://115.68.20.84/event/event_cartbanner_mastercard.jpg" alt="마스터카드이벤트" usemap="#Map" border="0" />
<map name="Map" id="Map">
  <area shape="rect" coords="4,4,337,117" href="<?=$g4['shop_path']?>/master_hanacard_event.php"  target='_blank' onfocus="blur();"/>
  <area shape="rect" coords="345,4,681,116" href="https://customer.kbcard.com/CXCEVZZC0012.cms?mainCC=a&amp;이벤트일련번호=260694" target='_blank' onfocus="blur();"/>
  <area shape="rect" coords="690,3,1026,117" href="<?=$g4['shop_path']?>/master_shinhancard_ihappy.php" target='_blank' onfocus="blur();"/>
</map>
</div>
...
if($_MASTER_CARD_EVENT && !in_array($s_page,array('orderform.php','orderinquiryview.php') ) && date('Ym') <= '201502'  ) {?>
<p><img src="http://115.68.20.84/event/master_card/master_samsung_cart.jpg"></p>
<?php }
...
<?php
if($s_page == 'orderform.php' && $notice_msg){
	echo "<p style='text-align: center;font-weight: bold;color: #ff0000;'>".$notice_msg."</p>";
}
?>