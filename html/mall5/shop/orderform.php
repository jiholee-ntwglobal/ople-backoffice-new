	...
	<tr>
		<td>
			...
			<tr>
				<th>이름</th>
				<td>
					<input type=text name=od_b_name class=ed maxlength=20>
					<p>
					반드시 주민등록상의 성함을 기재해 주셔야 하며, 잘못 기재 했을 시 통관지연등의 불이익을 받으실 수 있으니, 주의 바랍니다.
					</p>
				</td>
			</tr>
			<!-- // 김선용 200908 : -->
			...
			<tr>
				<td>
					...
					<input type="hidden" name="od_b_zonecode" value="<?=$member['mb_zonecode'];?>"/>
					<span id="od_b_addr_jibeon"></span>
				</td>
			</tr>
			<tr>
				<td>배송시 요청사항<br/>(ex.부재시 경비실에 맡겨주세요.)</td>
				...
				else
				{
					...
					*/
					...
					<div id='_dis_authorize_info_' style='display:none;'>
						...
						".($_MASTER_CARD_EVENT && date('Ym') <= '201502' ? "
						<div><img src='http://115.68.20.84/event/master_card/master_samsung_cart.jpg'/></div>
						":"")."
						...
						<div class='cardinfo'></div><br>
							...
							<b>1.</b> 해외 사용이 가능한 <u>Master, Visa, Amex, BC글로벌</u> 로고가 있는 카드만 가능합니다.<br>
...
function forderform_check(f)
{
	...
	check_field(f.od_tel, "주문하시는 분 핸드폰번호를 입력하십시오.");
	...
	switch($(this).val()){
		case 'c_code' :
			in_div =
				...
				"개인통관고유부호 : <input type='text' name='od_b_code' id='od_b_code' style='width:180px;' maxlength=13 class='ed' itemname='개인통관고유부호' disabled><br><br/><span class='warning'>"+
				"<b>받으시는분의 개인통관고유부호를 입력해 주세요.</b></span>";
			break;
		case 'jumin' :
			in_div =
				"<div class='' style='background-color:#fff;line-height:16px;padding:10px;font-size:11px;'>"+
				...
				"<div style='margin:10px 0;'>"+
					...
					"<p style='border-bottom:solid 1px #ccc;padding-bottom:10px;'><input type='checkbox' name='detail_clause_agree' value='Y' onchange=\"detail_clause_agree_change();\">동의합니다.</p>"+
				"</div>"+
				"주민등록번호 : <input type='password' name='od_b_jumin' id='od_b_jumin' style='width:180px;'  maxlength=13 class='ed' jumin itemname='받는사람 주민등록번호' disabled>('-'없이 숫자만 입력하세요)"+
				"<br/>외국인 : <input type='checkbox' class='foreigner' onclick=\"foreigner_check();\"/>"+
				"<br/><br/><span class='warning'><b>받으시는 분의 주민등록번호를 입력해 주세요</b></span>";
			break;
	}
