...
<?
    ...
    {
		// 김선용 2014.03 :
		$kcp_result = "0000"; // 처리완료 메시지, 미사용.
		$od = sql_fetch("select od_id, on_uid, mb_id, od_name, od_hp, od_temp_bank,od_dc_amount from {$g4['yc4_order_table']} where od_id='$order_no' ");

		if($od['od_dc_amount']){
			$od['od_temp_bank'] = $od['od_temp_bank'] - $od['od_dc_amount'];
		}

		if($od['od_temp_bank'] != $ipgm_mnyx)
		{
			$ipgm = $ipgm_mnyx;
			$temp = $od['od_temp_bank'];
			$kcp_result = "1111"; // 0000 이외는 처리완료가 아니라는 메시지.
		  ...
		  // 주문서 UPDATE
			...
			// 전자결제내역
			sql_query("update {$g4['yc4_card_history_table']} set cd_amount='$ipgm_mnyx', cd_opt01='$remitter' where od_id='$order_no' ");

			// 장바구니
			//sql_query("update {$g4['yc4_cart_table']} set ct_status='준비' where on_uid='{$od['on_uid']}' and ct_status in('주문') ");

		}
		else if($od['od_temp_bank'] == $ipgm_mnyx)
		{
	        // 주문서 UPDATE
			$sql = " update $g4[yc4_order_table]
						set od_receipt_bank = '$ipgm_mnyx',
			...
			# 중복 노티로 인한 중복 재고 차감 방지 2014-12-30 홍민기 #
			$pay_ct_status_chk = sql_fetch("select count(*) as cnt from ".$g4['yc4_cart_table']." where on_uid='{$od['on_uid']}' and ct_status in('주문')");

			if($pay_ct_status_chk['cnt'] > 0){
				$pay_complate = true;
			}

			$pay_confirm_dt_chk = sql_fetch("
				select count(*) as cnt from ".$g4['yc4_order_table']." where od_id = '".$order_no."' and od_bank_time is not null
			");

			if($pay_confirm_dt_chk['cnt']>0){
				$pay_complate = false;
			}else{
				$pay_complate = true;
			}
			...
			// 장바구니
			sql_query("update {$g4['yc4_cart_table']} set ct_status='준비',ct_status_update_dt = '".$g4['time_ymdhis']."' where on_uid='{$od['on_uid']}' and ct_status in('주문') ");
		...
		if ($default['de_sms_use3'])
		{
			...
			$sms_contents = preg_replace("/{입금액}/", nf($ipgm_mnyx), $sms_contents);
			$sms_contents = preg_replace("/{주문번호}/", $order_no, $sms_contents);
	...
	if($pay_complate && $od['on_uid']){
		sales_report_cart_update($od['on_uid'],null,$default['de_conv_pay']);
		clearance_sell_qty($od['on_uid']);
	}
?>
