...
<?}?>
        ...
        $sql_common = " from $g4[yc4_item_table] a left join $g4[yc4_category_table] b on a.ca_id=b.ca_id
			where a.it_use = 1 and a.it_discontinued=0 and b.ca_use = 1 ";
    ...
    */
    ...
    for ($i=0; $i<$save[cnt]; $i++) {
		...
		if(trim($_GET['search_str']) != '')
			...
			$it_name = search_font(stripslashes($_GET['it_maker']), stripslashes($row['it_name']));

        echo "
            <tr>
                <td align=center style='padding-top:7px; padding-bottom:7px;'>$image</td>
                <!--<td>&nbsp;".it_name_icon($row)."</td>-->
				<td>&nbsp;<a href='{$g4['shop_path']}/item.php?it_id={$row['it_id']} '>$it_name</a></td>
