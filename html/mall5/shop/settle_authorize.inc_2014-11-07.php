...
//$settle_test = true;
if(date('Ymd')>='20141110' && date('Ymd') <= '20141231' || $settle_test){
	$master_card_event = true;
	//kb vcn 사용 여부 체크
	$kb_vcn = true;
	$kb_vcn_chk = sql_fetch("select count(*) as cnt from master_card_event_kb_vcn where mb_id = '".$member['mb_id']."'");
	if($kb_vcn_chk['cnt']>0){
		$kb_vcn = false;
	}
}
...
<input type=hidden name="x_first_name" value="<?=substr($member['mb_name'],0,2)?>">
<input type=hidden name="x_last_name" value="<?=substr($member['mb_name'],2)?>">
...
<tr>
	<th>카드 번호</th>
	<td><input class=ed type='text' name="x_card_num" size="30" numeric required itemname='카드번호' maxlength=16 onchange='master_card_price_view(this.value)'> (숫자만 입력하십시오.)</td>
</tr>
...
<?php
if( $master_card_event ){ // 마스터 카드 프로모션 기간 한정 2014-10-16 홍민기
?>
// 마스터 카드 체크
...
function kb_vcn_chk(num){
	...
	// vcn bin값
	var vcn_arr = new Array('517633','538798','520982','510194');
...
// 할인 혜택 체크
function master_cart_pro(num){
	if(master_card_chk(num) == false){
		return false;
	}


	var send_cost = Number('<?php echo $send_cost;?>');
	var tot_amount = Number('<?php echo $od[od_temp_card];?>');

	var result_amount = tot_amount;

	var no_master_card_item_amount = Number('<?php echo $master_card_price;?>');
	...
	if(result_amount >= 100000){ // 10만원 이상 결제시 10% 할인
		result_amount = (result_amount - no_master_card_item_amount) * 0.9;
	}

	var x_amount = usd_convert(result_amount);
	return Array(result_amount,x_amount);
}
...
function master_card_price_view(num){
	var amount_arr = master_cart_pro(num);
	...
	if(sale_amount > 0){
		master_card_comment =
			...
			'<br/>'+
			'할인금액 : ' + number_format(String(sale_amount)) + "&nbsp;<span style='font-size:9pt; font-family:tahoma; color:#6666FF;'>($"+ sale_amount_usd +")</span>" + '<br/>' +
	...
	if(kb_vcn_point > 0){
		kb_vcn_comment = '<br/>'+
			...
			"KB-MASTER CARD 해외온라인 안전결제(VCN)으로 주문 후 수령확인시 결제금액(카드)의 10%를 포인트로 적립해 드립니다.(1인 1회 한정)" +
