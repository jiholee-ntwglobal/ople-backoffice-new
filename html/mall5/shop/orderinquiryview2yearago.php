...
$sql = $db->ople_backup->query("select * from $g4[yc4_order_table] where od_id = '$od_id' and on_uid = '$on_uid' ");
$od=$sql ->fetch_array();
...
$od_mb_point = $db->ople_db->query("select mb_point from ".$g4['member_table']." where mb_id = '".$od['mb_id']."' ")->fetch_array();
...
$delivery_chk = $db->ople_backup->query("select count(*) as cnt from ".$g4['yc4_cart_table']." where on_uid = '".$s_on_uid."' and ct_status in ('배송')")->fetch_array();
...
<tr>
	...
	if($od['od_gift_id'] != ''){
		...
		for($k=0; $k<count($gi_id); $k++){
			$gi = $db->ople_db->query("select gift_title from {$g4['yc4_gift_table']} where gift_id='{$gi_id[$k]}' ")->fetch_array();
...
<?
// 배송회사 정보
$dl = $db->ople_db->query(" select * from $g4[yc4_delivery_table] where dl_id = '$od[dl_id]' ")->fetch_array();
...
if ($od[od_settle_case] == '신용카드')
{
    if ($od[od_receipt_card] > 0)
    {
        ...
        $result = $db->ople_db->query($sql);
        $cd =$result->fetch_array();
    }
...
if ($tot_cancel_amount == 0) {
    if (($od[od_temp_bank] > 0 && $od[od_receipt_bank] == 0) ||
        ...
        echo "<table cellpadding=0 cellspacing=0 width=100% class='list_order'>";
        echo "<colgroup><col width='140'><col /></colgroup><tbody>";
        echo "<tr><td style='color:blue;'>주문 취소를 원하시는 경우 고객센터로 전화주시거나, 고객센터 시간이 마감된 이후는 고객센터 게시판에 취소를 요청해주셔야 합니다.<br/>주문건은 국내시간 새벽 2시부터 발송처리가 되어, 발송 전 주문건의 경우 게시판으로 알려주시면 취소요청건은 발송처리를 하지 않습니다.<br/>고객센터 (10:00~16:30) 070-7678-7004 </td></tr>";
        echo "</tbody></table>";
    }
} else {
