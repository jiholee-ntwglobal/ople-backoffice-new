...
if($s_type == '0') // 기본 1곳 입력폼
{
	...
	<colgroup>
      ...
      <tbody>
			{$bottle_str}
			<tr>
				<td colspan=2  style='text-align:left;height:25px;line-height:25px;padding:10px 0;border-top:solid 1px #666;'>
					<input type=checkbox id=same name=same onclick=\"javascript:gumae2baesong(document.forderform);\">
					<label for='same'><strong>주문하시는 분과 받으시는 분의 정보가 동일한 경우 체크<strong></label><input type=image src='img/btn_add_find.gif' value='기존배송지찾기' style='height:23px;vertical-align:middle' onclick=\"get_ship_addr(); return false;\" />
				</td>
			</tr>
			<tr>
				<th>이름</th>
				<td style='text-align:left;'><input type=text name=od_b_name id=od_b_name class=ed maxlength=20></td>
			</tr>
			<tr>
				<td><u>통관고유식별정보</u></td>
				<td>
					<input type='radio' name='customs_clearance_code' value='c_code' onclick='customs_clearance_code_change();'/> 개인통관고유부호
					<input type='radio' name='customs_clearance_code' value='jumin' onclick='customs_clearance_code_change();' /> 주민등록번호
			...
			<tr>
				<th>전화번호</th>
				<td style='text-align:left;'><input type=text name=od_b_tel id=od_b_tel class=ed maxlength=20></td>
			</tr>
			<tr>
				<th>핸드폰</th>
				<td style='text-align:left;'><input type=text name=od_b_hp id=od_b_hp class=ed maxlength=20></td>
			</tr>
			<tr>
				<th rowspan=3>주 소</th>
				<td style='text-align:left;'>
					...
					<a href=\"javascript:;\" onclick=\"win_zip('forderform', 'od_b_zip1', 'od_b_zip2', 'od_b_addr1', 'od_b_addr2', 'od_b_addr_jibeon' ,'od_b_zonecode');\"><img
			...
			<tr>
				<td style='text-align:left;'>
					<p><input type=text name=od_b_addr1 id=od_b_addr1 size=35 maxlength=50 class=ed readonly  style='width:705px;'></p>
					<p><input type=text name=od_b_addr2 id=od_b_addr2 size=15 maxlength=50 class=ed  style='width:505px;'> (상세주소)</p>
				</td>
			</tr>
			<tr>
				<td style='text-align:left;'>
					<input type='hidden' name='od_b_addr_jibeon'>
					<input type='hidden' name='od_b_zonecode'/>
					<span id='od_b_addr_jibeon'></span>
				</td>
			</tr>
			<tr>
				<th>전하실말씀</th>
				<td style='text-align:left;'><textarea name=od_memo rows=4 cols=60 class=ed></textarea></td>
			</tr>
      </tbody>
			</table>";
...
else if($s_type == '1') // 복수배송지 입력폼. 지정할상품
{
	...
	$ct_arr = array("<table width=100% cellpadding=0 cellspacing=0 border=0 class='list_styleA'>
		<colgroup>
      <col width=30>
		  <col width=50>
		  <col></col>
	...
	$return_arr[] = "
			<fieldset style='padding:10px 0; width:100%;'>
      ...
      <tbody>
			<tr>
				<td colspan=2  style='text-align:left;height:25px;line-height:25px;padding:10px 0;border-top:solid 1px #666;'>
					<input type=checkbox id=same name=same onclick=\"javascript:gumae2baesong(document.forderform);\">
					<label for='same'><strong>주문하시는 분과 받으시는 분의 정보가 동일한 경우 체크</strong></label> <input type=image src='img/btn_add_find.gif' value='기존배송지찾기' title='기존배송지찾기' onclick=\"get_ship_addr(); return false;\" style='height:23px;vertical-align:middle' />
				</td>
			</tr>
			<tr>
				<th>보내는사람(업체명등)</th>
				<td style='text-align:left;'><input type=text name=od_post_name id=od_post_name class=ed maxlength=20> ※ 최대 20자</td>
			</tr>
			<tr>
				<th>이름</th>
				<td style='text-align:left;'><input type=text name=od_b_name id=od_b_name class=ed maxlength=20></td>
			...
			<tr>
				<td><u>통관고유식별정보</u></td>
				<td>
					<input type='radio' name='customs_clearance_code' value='c_code' onchange='customs_clearance_code_change();'/> 개인통관고유부호
					<input type='radio' name='customs_clearance_code' value='jumin' onchange='customs_clearance_code_change();'/> 주민등록번호
					<div class='customs_clearance_code_wrap' style='margin-top:10px;'>
						<b style='color:#ff0000'>통관필수요건을 선택해 주세요.</b>
					</div>
				</td>
			...
			<tr>
				<td style='text-align:left;'>
					<p><input type=text name=od_b_addr1 id=od_b_addr1 size=35 maxlength=50 class=ed readonly style='width:705px;'></p>
					<p><input type=text name=od_b_addr2 id=od_b_addr2 size=15 maxlength=50 class=ed style='width:540px;'> (상세주소)</p>
				</td>
			</tr>
			<tr>
				<td style='text-align:left;'>
			...
			<tr>
				<td colspan=2  style='text-align:left;'><input type=button value='저장하기' title='저장하기' onclick=\"get_ship_save();\" style='height:30px;' /></td>
			</tr>
			<tr><td colspan=2  style='text-align:left;'><span style='color:blue;'>※ 배송지를 추가할경우, 입력하고 저장하기를 반복하면 계속 추가 입력이 가능합니다.</span></td></tr>
			</table>";
