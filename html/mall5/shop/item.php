...
# 직원 체크 #
$manager_sql = sql_fetch("select count(*) as cnt from manager_id where mb_id = '".$member['mb_id']."'");
if($manager_sql['cnt']>0){
	$manager_fg = true;
}

if($_GET['ep_code']){
	/* 네이버지식쇼핑에서 접근시 세션 처리 추가 - 2015-01-20 이성용 */
	if($_GET['it_id'] == ($_GET['ep_code']/3)) $_SESSION['naver_ep_it_id'] = $_GET['it_id'];

	/* 네이버 cpc 카운팅 추가 - 2015-03-24 이성용*/
	count_naver_cpc($_GET['it_id']);
}
...
if($_POST['mode'] == 'qty_chk'){
	...
	echo $result < 0 ? 0 : number_format($result);
	exit;
}
...
");
if($clearance_chk){
	$it['it_stock_qty'] = $clearance_chk['qty'];
}
if($clearance_chk['ori_it_id']){
	$clearance_ori_it = sql_fetch("
		select
			it_id,it_amount,it_tel_inq
		from
			".$g4['yc4_item_table']."
		where
			it_id = '".$clearance_chk['ori_it_id']."'
	");
}

# 기타 가격 로드 2015-03-04 홍민기 #
$ect_amount_it_id = $it_id;
if($clearance_chk['ori_it_id']){
	$ect_amount_it_id = $clearance_chk['ori_it_id'];
}

$etc_amount_sql = sql_query("
	select
		b.code_name as pay_name,
	...
	from
		...
		left join
		yc4_code b on a.pay_code = b.code_value and b.code_type = 'pay'
		left join
		yc4_code c on a.money_type = c.code_value and b.code_type = 'money'
	where
		it_id = '".$ect_amount_it_id."'
");
$ect_amount_tr = '';
while($row = sql_fetch_array($etc_amount_sql)){
	switch($row['money_type']){
		case 'krw' : 
			$etc_pay = "￦ ".number_format($row['amount'])." ($ ".number_format(usd_convert($row['amount'])).")"; 
			break;
		case 'usd' :
			$etc_pay = "￦ ".number_format($row['amount'] * $default['de_conv_pay'])." ($ ".$row['amount'].")";
			break;
	}
	$ect_amount_tr .= "
		<tr>
...
if($_SERVER['REMOTE_ADDR'] == '59.17.43.129'){
	$nonstop_event_item = sql_fetch("select it_id,ev_amount from yc4_nontop_sale where status=2 and it_id='$it_id'");

	if($nonstop_event_item['ev_amount'] > 0) {
		$it['it_amount'] = $nonstop_event_item['ev_amount'];
...
$tongawn_msg_sql = sql_query("
	...
	from
		yc4_category_item a
		left join
		yc4_item_tongwan b on a.it_id = b.it_id,
...
if($oneday_data){
	...
	if(
		...
		$gap = mktime(23,59,59,$oneday_month,$oneday_day,$oneday_year)-time();
		/*
		$sql = " select a.*,
					b.ca_name,
					b.ca_use
...
# 셋트상품 가격 정보 표시 2015-02-03 홍민기 #
$set_item_chk = set_item_ori_amount($it_id);

$child_it_id_in = '';
if(count($set_item_chk['child_it_id_arr']) > 0){
	$child_it_id_in = implode(',',$set_item_chk['child_it_id_arr']);

	# 셋트상품 상품설명 합치기 #
	if($child_it_id_in && $it_id !='1407396665'){
		$tmp_it = sql_fetch("
			select it_explan from ".$g4['yc4_item_table']." where it_id in (".$child_it_id_in.")
		");
		$it['it_explan'] .= $tmp_it['it_explan'];
...
<tr>
	...
	//}
    $clearance_dc_ico = '';
    if($clearance_chk['msrp']){
        $clearance_dc_ico = "<span class='ico_off_arrow' style='text-align: center; right:-80px;'>".get_dc_percent(usd_convert(get_amount($it)),$clearance_chk['msrp'])."%</span>";
    }
    ...
    <td width=405 align=center valign=top style='border-right:solid 1px #eee;'>
        <table cellpadding=0 cellspacing=0 width=100%>
            <tr><td align=center style='padding:35px 0 10px 0;'>
				...
				<span style="position:absolute;top:-40px;left:-85px;"><img src="img/ico_onedaysale.png" alt="원데이세일"></span><?=get_large_image($it[it_id]."_l1", $it[it_id], false)?><?php echo get_it_image($middle_image, 200, 200);?></a></td></tr></table></td></tr>
				...
				<?php }else{?>
                <table cellpadding=0 cellspacing=0><tr><td style='position: relative;'><?php echo $clearance_dc_ico;?><?php echo get_large_image($it['it_id']."_l1", $it['it_id'], false)?><?php echo get_it_image($middle_image, 200, 200,null,"onclick=\"return false;\"",false,false);?></a></td></tr></table></td></tr>
				<?php }?>
            <tr>
...
//                        echo "</a> &nbsp;";
    ...
    <td valign=top align=center style='padding:20px 40px;'>
		...
		if($it['SKU'] && (is_admin($member['mb_id']) || $adm_chk['cnt']>0)) {
			$mapping_sql = sql_query("select * from ople_mapping where it_id = '".$it_id."'");
			$upc_arr = $upc_in = '';
			$upc_data = array();
			...
			if($NTICS_DATA_ON){
				...
				if($upc_in){
					...
					foreach($json_data as $upc => $val){
						...
						if(isset($val->amount->iherb)){
							$item_info[$upc]['iherb_amount'] = $val->amount->iherb;
						}else{
							$item_info[$upc]['iherb_amount'] = null;
						}
						if(isset($val->msrp->iherb)){
							$item_info[$upc]['msrp'] = $val->msrp->iherb;
						}else{
							$item_info[$upc]['msrp'] = null;
						}
						if(isset($val->msrp->iherb)){
							$item_info[$upc]['msrp'] = $val->msrp->iherb;
						}else{
						...
						if(isset($val->amount_dt->iherb)){
							$item_info[$upc]['amount_dt'] = $val->amount_dt->iherb;
						}else{
							$item_info[$upc]['amount_dt'] = null;
						}
					}
				}
			}

			foreach($upc_data as $val){
				$item_amount_data_add = '';
				if($item_info[$val['upc']]['msrp']){
					$item_amount_data_add .= "<br/>└ MSRP : $ ".number_format($item_info[$val['upc']]['msrp'],2)." (￦ ".number_format($item_info[$val['upc']]['msrp'] * $default['de_conv_pay']).")";
				}
				if($item_info[$val['upc']]['iherb_amount']){
					$item_amount_data_add .= "<br/>└ 아이허브가 : $ ".number_format($item_info[$val['upc']]['iherb_amount'],2)." (￦ ".number_format($item_info[$val['upc']]['iherb_amount'] * $default['de_conv_pay']).")";
				}
				if($item_info[$val['upc']]['amount_dt']){
					$item_amount_data_add .= "<br/>└ 기준시간 : ".date('Y-m-d H:i:s',strtotime($item_info[$val['upc']]['amount_dt']))."";
				}

				$upc_arr .= "
					<p>
						".$val['upc']." x ".$val['qty']."ea / NTICS QTY : ".number_format($item_info[$val['upc']]['currentqty'])." 
						".$item_amount_data_add."
					</p>
				";
			}
        ...
        <?php if (!$it[it_gallery]) { // 갤러리 형식이라면 가격, 구매하기 출력하지 않음 ?>
            ...
            <?php } else { ?>
				...
				<?php if($it['it_amount_dutyfree']){?>
				<tr>
					<th>면세점가</th>
					<td>
						￦ <span style='text-decoration:line-through;'><?php echo number_format($it['it_amount_dutyfree']);?></span>
						<span style='text-decoration:line-through;'>($ <?php echo number_format(usd_convert($it['it_amount_dutyfree']),2);?>)</span>
						<span style='color:#ff0000;'>
						<?php echo (100 - round(get_amount($it) / $it['it_amount_dutyfree']  * 100));?>% OFF
						</span>
					</td>
				</tr>
				<?php }?>
				...
				<?php if($it['it_amount_sijoong'] && !$it['it_amount_dutyfree']){?>
				<tr>
					<th>시중가</th>
					<td>
						￦ <span style='text-decoration:line-through;'><?php echo number_format($it['it_amount_sijoong']);?></span>
						<span style='text-decoration:line-through;'>($ <?php echo number_format(usd_convert($it['it_amount_sijoong']),2);?>)</span>
						<span style='color:#ff0000;'>
						<?php echo (100 - round(get_amount($it) / $it['it_amount_sijoong']  * 100));?>% OFF
						</span>
					</td>
				</tr>
				<?php }?>
				...
				<tr>
					<th>단품구매가격</th>
					<td>
						￦ <span style='text-decoration:line-through;'><?php echo number_format($set_item_chk['ori_it_amount']);?></span>
						<span style='text-decoration:line-through;'>($ <?php echo number_format(usd_convert($set_item_chk['ori_it_amount']),2);?>)</span>
						<span style='color:#ff0000;'>
						<?php echo $set_item_chk['dc_per'];?>% OFF
						</span>
					</td>
				</tr>
				<?}elseif($new_year_item_set[$it_id]){?>

				<tr>
					<th>정상가</th>
					<td>
						￦ <span style='text-decoration:line-through;'><?php echo number_format($new_year_item_set[$it_id]);?></span>
						<span style='text-decoration:line-through;'>($ <?php echo number_format(usd_convert($new_year_item_set[$it_id]),2);?>)</span>
						<span style='color:#ff0000;'>
						<?php echo get_dc_percent(get_amount($it),$new_year_item_set[$it_id]);?>% OFF
						</span>
					</td>
				</tr>
				<?}?>
                <tr>
                    <th>
						<?if($new_year_item_set[$it_id]) {?>
						<strong>설맞이 특가</strong>
						<?}else{?>
						판매가격
						<?}?>
					</th>
                    <td>
						...
						<?=number_format(usd_convert(get_amount($it)),2);?>
						</span>)
                    </td>
                </tr>
				<?php if($manager_fg){?>
				<tr>
                    <th>일반회원가</th>
                    <td><strong class='text_won'>￦</strong> <?=number_format($it['it_amount']);?>
                    </td>
                </tr>
				<?php }?>
				...
				<?php
				# 클리어런스 상품 정보 시작 2015-02-26 홍민기 #
				if($clearance_ori_it){
				?>
				<tr>
					<td>정상가</td>
					<td>
						￦ <?=number_format(get_amount($clearance_ori_it));?> ($ <?php echo number_format(usd_convert(get_amount($clearance_ori_it)),2);?>)
					</td>
				</tr>
				<?php
				}
                ...
                <tr>
                    <th>미국 권장 소비자가</th>
                    <td>￦ <?php echo number_format($clearance_chk['msrp'] * $default['de_conv_pay'])?> ($ <?php echo number_format($clearance_chk['msrp'],2)?>)</td>
                </tr>
                <?php
                }
				if($clearance_chk['exp_date']){
				?>
				<tr>
					<th>유통기한</th>
					<td><?php echo $clearance_chk['exp_date'];?></td>
				</tr>
				<?php
				}
				if($clearance_chk['comment_column_nm']){
				?>
				<tr>
					<th><?php echo $clearance_chk['comment_column_nm'];?></th>
					<td><?php echo $clearance_chk['comment_column_val'];?></td>
				</tr>
				<?php }
				# 클리어런스 상품 정보 끝 #
				?>
                ...
                */
				...
				<?php
				// 품절 sms통보
				if($it['it_stock_qty'] < 1 && !$clearance_chk) {
        ...
        <tr>
            ...
            <a onclick="fitemcheck(document.fitem, 'direct_buy'); return false;" href="#"><!--<img src='<?=$g4[shop_img_path]?>/btn2_now_buy.gif' border=0>--><img src="<?php echo $g4['path'];?>/images/category/category_buy_box01_btn_buy.gif" border="0"></a>
            <a onclick="ajax_cart_update(); return false;" href="#"><!--<img src='<?=$g4[shop_img_path]?>/btn2_cart.gif' border=0>--><img src="<?php echo $g4['path'];?>/images/category/category_buy_box01_btn_cart.gif" border="0"></a>
            <?php } ?>

            <? if (!$it['it_gallery']) { ?>
            <a onclick="item_wish(document.fitem, '<?php echo $it['it_id'];?>'); return false;" href="#"><!--<img src='<?=$g4[shop_img_path]?>/btn2_wish.gif' border=0>--><img src="<?php echo $g4['path'];?>/images/category/category_buy_box01_btn_wish.gif" border="0" style='margin-left:15px;'></a>
            <a onclick="popup_item_recommend('<?php echo $it['it_id'];?>'); return false;" href="#"><!--<img src='<?=$g4[shop_img_path]?>/btn_item_recommend.gif' border=0>--><img src="<?php echo $g4['path'];?>/images/category/category_buy_box01_btn_friend.gif" border="0"></a>
            <? } ?>
...
<?php if($_SERVER['REMOTE_ADDR'] == '59.17.43.129' && $child_it_id_in){ ?>
<div id="item_set">
    <h2>상품구성</h2>
    <table width="100%" cellpadding="0" cellspacing="0" class="list_styleB">
        <col width="120"/>
        <col/>
        <col width="100"/>
        <col width="100"/>
    ...
    <?php
    # 수량 로드 #
    $set_it_info = sql_fetch("select group_concat(concat(child_it_id,'*',child_qty)) as data from yc4_item_set where it_id = '".$it_id."'
group by it_id");
    $child_it_id_qty = explode(',',$set_it_info['data']);
    if(is_array($child_it_id_qty)){
        $child_it_id_data = array();
        foreach($child_it_id_qty as $val){
            list($tmp_it_id,$tmp_qty) = explode('*',$val);
            $child_it_id_data[$tmp_it_id] = $tmp_qty;
        }
    }
    $set_it_sql = sql_query("
      select it_id,it_name,it_amount,it_maker
      from
        ".$g4['yc4_item_table']."
      where
        it_id in ($child_it_id_in)
    ");
    while($row = sql_fetch_array($set_it_sql)){
        $row_amount = get_amount($row);
        echo "
            <tr>
                <td>".get_it_image($row['it_id'].'_s',50,50,$row['it_id'],null,null,ture,true)."</td>
                <td style='text-align:left;'>".get_item_name($row['it_name'])."</td>
                <td>".$child_it_id_data[$row['it_id']]."</td>
                <td>￦".number_format($row_amount)."<br />($".usd_convert($row_amount).")</td>
                <td>￦".number_format($row_amount*$child_it_id_data[$row['it_id']])."<br />($".usd_convert($row_amount*$child_it_id_data[$row['it_id']]).")</td>
            </tr>
        ";
    }
    ?>
...
if(isset($set_item_chk['child_it_id_arr'])){
    if(is_array($set_item_chk['child_it_id_arr'])){
        foreach($set_item_chk['child_it_id_arr'] as $val){
            $file = 'http://115.68.20.84/desc/'.$val.'.jpg';
            $file_headers = @get_headers($file);
            if($file_headers[0] == 'HTTP/1.1 404 Not Found') {
                $tmp_it_explan = sql_fetch("select it_explan from ".$g4['yc4_item_table']." where it_id = '".$val."'");
                echo conv_content($tmp_it_explan['it_explan'], 1);
            }
            else {
    ...
    if ($_GET['it_id'] == '1422599151') {
        $file = 'http://115.68.20.84/desc/1413898640.jpg';
    } else {
...
$qa_page_rows = 30;     // 상품문의 페이지당 목록수
	if($customer_question) {
		include_once($g4['full_shop_path'] . "/itemqa.inc02.php");
		//곽범석 추가
	}else{
...
function amount_change()
{
    ...
    var basic_point  = parseFloat('<?=$it[it_point]?>');
    var cust_amount  = parseFloat('<?=$it[it_cust_amount]?>');
	var usd_amount = parseInt('<?=number_format(usd_convert(get_amount($it)),2);?>');
