...
//        $continue_ca_id = $row[ca_id];
	...
	<td>
		<span class='amount'>
			...
			(<span>$</span>" . number_format(usd_convert($row['ct_amount']),2).")
		</span>
	</td>
	";
    echo "
	<td>
		<span class='amount'>
			...
			(<span>$</span>" . number_format(usd_convert($sell_amount),2).")
		</span>
	</td>
	";
...
if ($i == 0) {
    ...
    if ($send_cost > 0)
    {
		...
		echo "<td colspan='4' rowspan='3' style='width:735px;'>".($ov_evt_flag  ? "※ +1 이벤트 사은품은 주문서 작성 완료 후 주문내역에 추가됩니다.":"")."</td>";
	...
	<td colspan=3 style='text-align:right;padding-right:10px;'>
		...
		($ ".number_format(usd_convert($tot_amount),2).")
	</td>";
