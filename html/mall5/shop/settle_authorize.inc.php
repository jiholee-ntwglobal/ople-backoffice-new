...
if($_MASTER_CARD_EVENT){
	...
	if($it_maker_in){
		...
		$no_master_card_tot_amount['amount'] = 0;
	}
	*/
...
<input type=hidden name="u_amount" value="<?=($od['od_temp_card']- $od['od_dc_amount']);?>">
...
<tr>
	...
	<div style='line-height:150%; background-color:#f3f3f3; padding:15px;'>
		...
		<b>3)</b> 카드결제는 해외 사용이 가능한 마스타/비자/아멕스(American express) 카드만 가능합니다.&nbsp;&nbsp;(카드에 마스타/비자/아멕스 로고가 있어야 합니다.)<br>
		<b>4)</b> 한국 국내전용 카드는 결제하실 수 없습니다.(Local Card)<br>
		<b>5)</b> 마스타/비자 카드라해도 해외사용 신청을 하지 않았으면 결제가 불가능합니다.<br>
		(카드사에 해외 사용신청이 되어있는지 확인하시기 바랍니다.)<br>
		<b>6)</b> 한국에서 사용하는 <u>체크카드</u>의 경우, 해외사용이 가능한 체크카드라면 결제가 가능합니다.(master/visa/jcb/discover 로고가 있는 체크카드)

	</div>
	<?if($_MASTER_CARD_EVENT && date('Ym') <= '201502'){ ?>
	<div><img src="http://115.68.20.84/event/master_card/master_samsung_cart.jpg" alt="" /></div>
	<?}?>
	</td>
</tr>
<tr>
	<th>카드 번호</th>
	<td><input class=ed type='text' name="x_card_num" size="30" numeric required itemname='카드번호' maxlength=16 onblur='master_card_price_view(this.value);hanacard_chk();'> (숫자만 입력하십시오.)</td>
</tr>
...
<!--
var x_amount = Number('<?=$_POST['x_amount'];?>'); // 원래 총 결제액(달러)
...
function master_card_price_view(num){
	...
	var result_tot_amount = Number('<?php echo $result_tot_amount;?>'); // 제외상품을 제외한 카드결제 금액
	x_amount = Number(usd_convert(tot_amount)); // 원래 총 결제액(달러)
...
var samsung_card_bin_array = new Array('510003','512365','517662','517709','517827','518319','520025','522855','527257','527419','529899','531070','531072','531080','531082','531085','533827','540447','540537','540538','541145','542184','545089','548869','552014','552412','553147','553176','558749','558953','558978','518831','536142','536148','536181','536648','552194');

var hanacard_bin_array = new Array('516450', '524180', '524242', '528523', '532147', '537725', '538825', '541707', '544174', '546252', '552125', '552323', '552407', '636189', '529875', '553173', '558738', '543617', '540497', '518283', '518185', '524335', '549861', '524154', '540799', '537733', '538833', '552133');

function hanacard_chk(){

    <?php if($hana_event_fg == false){?>
    return false;
    <?}?>

    var u_amount = AIMform.u_amount.value;

    // 하나카드 체크 and 5만원 이상
    if(hanacard_bin_array.indexOf(AIMform.x_card_num.value.substring(0,6)) >= 0 && u_amount >= 50000){
        $('.interest_wrap').show().find('select[name=interest]').removeAttr('disabled');

    }else{
        $('.interest_wrap').hide().find('select[name=interest]').val('0').attr('disabled',true);
    }

}


function samsung_chk(){

	<?php if(date('Ym') > '201502'){?>
	return false;
	<?}?>
	// 삼성카드 체크 and 49달러 이상
	if(samsung_card_bin_array.indexOf(AIMform.x_card_num.value.substring(0,6)) >= 0 && x_amount >= 49){
		$('.interest_wrap').show().find('select[name=interest]').removeAttr('disabled');
