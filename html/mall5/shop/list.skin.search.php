...
for ($i=0; $row=sql_fetch_array($result); $i++)
{
	...
	if(trim($_GET['search_str']) != ''){
		$it_name = search_font(stripslashes($_GET['search_str']), stripslashes(get_item_name($row['it_name'],'list')));
	}else if(trim($_GET['it_maker']) != ''){
		$it_name = search_font(stripslashes(trim($_GET['it_maker'])), stripslashes(get_item_name($row['it_name'],'list')));
	}
