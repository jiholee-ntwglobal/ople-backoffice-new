...
{ 
    ...
    while (list($key, $row) = each ($arr)) 
    {
        ...
        if ($save_ca_id != $row[ca_id]) 
        {
            ...
            echo "
            <tr height=20>
                <td style='border-bottom:2px solid #fa5a00;padding-top:40px;margin-bottom:20px;' colspan=4>&nbsp;<img src='$g4[shop_img_path]/icon3.gif'> <b><a href='./list.php?ca_id=$row[ca_id]'>$tmp_ca[ca_name]</a></b></td>
            </tr>";
        ...
        <tr>
            <td>
                ...
                <tr>
                    <td width=".($img_width+10)." height=".($img_height+10).">".get_it_image($row[it_id]."_s", $img_width , $img_height, $row[it_id])."</td>
                    <td class='sz14'><a href='./item.php?it_id=$row[it_id]' class=item>".it_name_icon($row,$row['it_name'],1,'list')."</a></td>
                </tr>
                </table>
            </td>
            ...
            if (!$row[it_gallery]) 
            {
                echo "<span class='sz16 amount'>".display_amount(get_amount($row), $row[it_tel_inq])."</span>";
...
?>
<tr><td colspan=4 style='border-bottom:1px solid #D4D4D4;padding-top:40px'></td></tr>
</table>
