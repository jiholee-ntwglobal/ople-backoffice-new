...
# 구글 애널리틱스 전자상거래 구매 상품 출력 2015-01-27 홍민기 #
$google_ecommerce_item_sql = sql_query("
	select
		a.it_id,a.ct_amount,a.ct_qty,
		b.it_name
	from
		".$g4['yc4_cart_table']." a,
		".$g4['yc4_item_table']." b
	where
		...
		a.on_uid = '".$on_uid."'
");
$google_ecommerce_item_add = '';
while($row = sql_fetch_array($google_ecommerce_item_sql)){
	$google_ecommerce_item_add_ca_name = sql_fetch("
		select
			concat(a.ca_id,'_',b.ca_name) as ca_name
		from
			yc4_category_item a,
		...
		limit 0,1
	");
	$google_ecommerce_item_add_ca_name = $google_ecommerce_item_add_ca_name['ca_name'];
	$google_ecommerce_item_add .= "
ga('ecommerce:addItem', {
  'id': '".$od_id."', //시스템에서 생성된 주문번호. 필수.
  'name': '".addslashes(get_item_name($row['it_name']))."', // 제품명. 필수.
  'sku': '".$row['it_id']."', // SKU 또는 제품고유번호. 선택사항.
  'category': '".$google_ecommerce_item_add_ca_name."', // 제품 분류.
  'price': '".$row['ct_amount']."', // 제품 단가.
  'quantity': '".$row['ct_qty']."' // 제품 수량.
});
	";
}
?>
...
switch($rand_ev_chk['ev_code']){
	case '1' : case '3' : case '4' : $ev_comment = "고디바 이벤트"; break;
...
ga('ecommerce:addTransaction', {
  'id': '<?php echo $od_id;?>', // 시스템에서 생성된 주문번호. 필수.
  'revenue': '<?php echo $settle_amount;?>', // 구매총액. 필수.
  'shipping': '<?php echo $od_send_cost;?>' // 배송비. 선택사항.
});



<?php echo $google_ecommerce_item_add;?>
