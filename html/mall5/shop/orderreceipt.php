...
<table width=100% align=center cellpadding=0 cellspacing=10 border=0>
	...
	<tr>
        ...
        <tr height=25>
            <td>주소</td>
            <td>
				<? echo sprintf("(%s-%s) %s %s", $_POST['od_zip1'], $_POST['od_zip2'], $_POST['od_addr1'], $_POST['od_addr2']); ?>
				<?if($_POST['od_addr_jibeon'] && $_POST['od_addr_jibeon'] != 'R'){?>
				<br />지번주소 : <?=$_POST['od_addr_jibeon']?>
				<?}?>
			</td>
        </tr>
        <tr height=25>
...
<table width=100% align=center cellpadding=0 cellspacing=10 border=0>
	...
	<tr>
        ...
        <tr height=25>
            <td>주소</td>
            <td>
				<? echo sprintf("(%s-%s) %s %s", $_POST['od_b_zip1'], $_POST['od_b_zip2'], $_POST['od_b_addr1'], $_POST['od_b_addr2']); ?>
				<?if($_POST['od_b_addr_jibeon'] && $_POST['od_addr_jibeon'] != 'R'){?>
				<br />지번주소 : <?=$_POST['od_b_addr_jibeon']?>
				<?}?>
    ...
    <table width=100% align=center cellpadding=0 cellspacing=10 border=0>
        ...
        <tr>
            ...
            if ($od_settle_case == "가상계좌")
            {
                ...
                echo "<tr><th>가상계좌</th><td><span class='amount'>".display_amount($receipt_amount)." <em>원</em></span>(결제하실 금액)</td></tr>";
                $receipt_amount = 0;
            }
            ...
            if ($od_settle_case == "에스크로")
            {
                ...
                echo "<input type=hidden name=od_receipt_bank value='$receipt_amount'>";
                //echo "<input type=hidden name=vbank_fix value='{$_POST['vbank_fix']}' />"; // 김선용 2014.04 : 고정계좌 구분
                echo "<tr><th>에스크로</th><td><span class='amount'>".display_amount($receipt_amount)." <em>원</em></span>(결제하실 금액)</td></tr>";
                $receipt_amount = 0;
            }
            ...
            if ($od_settle_case == "신용카드")
            {
                ...
                if($card_settle_case == 'authorize')
                {
                    ...
                    echo "<tr><th>신용카드</th><td><span class='amount'>".display_amount($receipt_amount)." {$dis_usd} <em>원</em></span>(결제하실 금액)</td></tr>";
                    $receipt_amount = 0;
                }
                else if($card_settle_case == 'kcp')
                {
                    ...
                    if ($od_receipt_bank == "") $border_style = " border-style:none;";
                    echo "<input type=hidden name=od_receipt_card value='$receipt_amount'>";
                    echo "<tr><th>신용카드</th><td><span class='amount'>".display_amount($receipt_amount)." <em>원</em></span>(결제하실 금액)</td></tr>";
                    $receipt_amount = 0;
                }
