...
$solr_str = $_GET['search_str'];
$solr_str = $solr_str2 = str_replace($str_bf_arr,$str_ch_arr,$solr_str);

$solr_str = urlencode($solr_str);

$solr_str_result = '*'.$solr_str.'*';
/*
$solr_str_result .= ' and '.$solr_str;
$solr_str_result = urldecode($solr_str_result);
$solr_str_result = urlencode($solr_str_result);
*/



$solr_result = file_get_contents('http://209.216.56.104:8080/solr/collection2/select?q={!q.op=AND%20df=it_name}'.$solr_str_result.'&wt=json&indent=true&rows=6000&facet=on&facet.field=maker');
//$solr_result = file_get_contents('http://209.216.56.104:8080/solr/collection2/select?q='.$solr_str.'&wt=json&df=it_name&indent=true&rows=6000&facet=on&facet.field=maker');

//$solr_result = file_get_contents('http://209.216.56.104:8080/solr/collection2/select?q={!q.op=AND%20df=it_name}*'.$solr_str.'*&wt=json&indent=true&facet=on&facet.field=maker');

//$solr_result = file_get_contents('http://209.216.56.104:8080/solr/collection2/select?q='.$solr_str_result.'&df=it_name&wt=json&indent=true&rows=6000&facet=on&facet.field=maker');

$solr_result_arr = json_decode($solr_result);

$id_arr = '';
foreach($solr_result_arr->response->docs as $key => $val){
	$id_arr .= ($id_arr ? ",":"")."'".$val->id."'";
}

$solr_str_result = $solr_str2;
$solr_str_result_arr = explode(' ',$solr_str_result);
foreach($solr_str_result_arr as $val){
	$solr_str_result2 .= ($solr_str_result2 ? " and ":"" )."it_name:*".$val."*";
}


$solr_str_result2 = urldecode($solr_str_result2);
$solr_str_result2 = urlencode($solr_str_result2);
$solr_result = file_get_contents('http://209.216.56.104:8080/solr/collection2/select?q='.$solr_str_result2.'&df=it_name&wt=json&indent=true&rows=6000&facet=on&facet.field=maker');
$solr_result_arr = json_decode($solr_result);
foreach($solr_result_arr->response->docs as $key => $val){
	$id_arr .= ($id_arr ? ",":"")."'".$val->id."'";
}
...
if(!$id_arr){
	$id_arr = '99999999'; // 검색어 없을시 처리
}
$maker_arr = $solr_result_arr->facet_counts->facet_fields->maker;
$mk_fg = false;
foreach($maker_arr as $val){
	if($continue){
		$continue = false;
		continue;
	}
	if(trim($val) !== ''){
		...
		if(!$mk_fg){
			$mk = ucfirst(trim(strtolower($val)));
			$mk_fg = true;
		}else{
			if($val > 0){
				$maker_result_arr[$mk] += $val;
			}
			$mk_fg = false;
		}

		//$maker_result_arr[] = $val;
...
//print_r2($solr_result_arr->response->docs);
...
//		$search_str2 = "+{$search_str}*";
		...
		$sql_where .= ($sql_where ? " and ":"") . " (a.it_id in (".$id_arr.") or a.it_name = '".$search_str."')";
	}
	...
	if(is_array($_GET['it_maker_arr'])){
		foreach($_GET['it_maker_arr'] as $val){
			$maker_where .= ($maker_where ? ",":"")."'".mysql_real_escape_string($val)."'";
		}
		if($maker_where){
			$maker_where = " and a.it_maker in (".$maker_where.")";
			$sql_where .= $maker_where;
		}
	}
...
if(is_array($maker_result_arr)){
	$i=0;
	foreach($maker_result_arr as $it_maker_nm => $cnt){
		$i++;
		if($i == 1){
			$it_maker_list.= "<tr>";
		}
		$it_maker_list.= "
			...
			<td>
				<span class='search_cate search_cate_1depth'><input type='checkbox' name='it_maker_arr[]' value=\"".$it_maker_nm."\" ".(in_array($it_maker_nm,$_GET['it_maker_arr']) ? "checked":"")."/>".$it_maker_nm." <em>".$cnt."</em></span>
			</td>
