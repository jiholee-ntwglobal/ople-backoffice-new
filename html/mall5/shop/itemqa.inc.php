...
<tr><td style='padding:15px'>
        ...
        // 테이블의 전체 레코드수만 얻음
        $sql = " select COUNT(iq_id) as cnt " . $sql_common;
        $row = sql_fetch($sql);
		...
		*/
		if($is_admin){
			$order_by = "ans desc,iq_time desc";
		}else{
			$order_by = "ans asc,iq_time desc";
		}
		# 자신의 글은 답변이 안달려도 보이며 답변에 상관없이 날짜순으로 정렬되고 그 이외의 글은 답변이 달리지 않는다면 무조건 뒤로 보낸다
		# 관리자는 미답변 글이 위에 보인다
		# 답변이 달리지 않은 문의는 관리자 및 작성자만 질문 내용을 확인할 수 있다. 2014-09-18 홍민기
		$sql = "
			select 
				if(iq_answer = '' and mb_id != '".$member['mb_id']."',1,0) as ans, 
				iq_id,it_id,mb_id,iq_name,iq_password,iq_subject,iq_question,iq_answer,iq_time,iq_ip
            $sql_common
                 order by ".$order_by."
			limit $qa_from_record, $qa_page_rows ";
        ...
        for ($i=0; $row=sql_fetch_array($result); $i++)
        {
			...
			if(!$row['iq_answer'] && !$is_admin){
				if($row['mb_id'] == $member['mb_id']){
					$iq_subject  = conv_subject($row[iq_subject],50,"…");
					$iq_question = conv_content($row[iq_question],0);
					$onclick = "onclick=\"qa_menu('iq$i')\"";
					$a_link = "<b><a href='javascript:;' ".$onclick."><b>$iq_subject</b></a></b>";
				}else{
					$iq_subject  = conv_subject('답변 대기중입니다.',50,"…");
					$iq_question = conv_content('',0);
					$onclick = "";
					$a_link = "<b>$iq_subject</b>";
				}
			}else{
				$iq_subject  = conv_subject($row[iq_subject],50,"…");
                if(!$iq_subject){
                    $iq_subject = conv_subject($row[iq_question],50,"…");
                }
				$iq_question = conv_content($row[iq_question],0);
				$onclick = "onclick=\"qa_menu('iq$i')\"";
				$a_link = "<b><a href='javascript:;' ".$onclick."><b>$iq_subject</b></a></b>";
			}
			//$iq_subject = cut_str(stripslashes($row[iq_subject]), 50, '..');
             $iq_question = conv_content($row[iq_question],0);
            ...
            if ($row[iq_answer])
            {
                ...
                $icon_answer = "<a href='javascript:;' ".$onclick."><img src='$g4[shop_img_path]/icon_answer.gif' border=0></a>";
            }
        ...
        $qa_pages = get_paging($qa_page_rows, $qa_page, $qa_total_page, "./item.php?it_id=$it_id&$qstr&qa_page=", "#qa");
        if ($qa_pages)
        {
            echo "<tr><td colspan=3 background='$g4[shop_img_path]/dot_line.gif'></td></tr>";
            echo "<tr>";
		...
		<!-- // 김선용 200908 : -->
        <tr bgcolor=#fafafa>
            <td align=right><img id='kcaptcha_image_qa' /></td>
            <td>
