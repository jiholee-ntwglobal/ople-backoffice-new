...
sql_query("alter table {$g4['config_table']}
	add cf_return_ip text not null,
	add cf_return_url varchar(255) not null", false);

// 김선용 200804 : 포인트 카운트 부하처리
sql_query("alter table {$g4['config_table']} add cf_point_sum int not null", false);

// 김선용 201005 :
sql_query("alter table {$g4['config_table']} add cf_ssl_use tinyint(4) not null", false);
...
<tr class='ht'>
	...
	<? // 김선용 200804 : 포인트테이블 부하 처리?>
	<td><span style="color:red;"><u>현재 포인트 카운트</u></span></td>
	<td><input type=text class=ed name='cf_point_sum' size='10' value='<?=$config['cf_point_sum']?>'> <?=help("※ 김선용 200804 : 수동으로 수정하지 마십시오.<br><br>포인트 입/출력에 따른 포인트 테이블 레코드 합계입니다.");?></td>
</tr>
<tr class='ht'>
    <td>로그인시 포인트</td>
...
<tr class='ht'>
	...
	<!-- // 김선용 201005 : -->
	<td><span style="color:red;">SSL 설정</span></TD>
	<td><input type="checkbox" name="cf_ssl_use" id="cf_ssl_use" value=1 <?if($config['cf_ssl_use']) echo 'checked';?>><label for="cf_ssl_use">사용</label>
</tr>
<tr class='ht'>
    <td>별명 수정</td>
...
<!-- 김선용 200801 -->
<tr class='ht'>
    <td>리턴 URL IP<br>접속도메인<br>리턴 URL</td>
    <td colspan=3 valign=top>
	<table width=100% cellpadding=0 cellspacing=0>
	<tr>
		<td width=80% align=left valign=bottom>
			<span style="cursor: pointer;" onclick="textarea_decrease('cf_return_ip', 5);"><img src="<?=$g4['path']?>/adm/img/btn_up2.gif"></span>
			<span style="cursor: pointer;" onclick="textarea_original('cf_return_ip', 5);"><img src="<?=$g4['path']?>/adm/img/btn_init2.gif"></span>
			<span style="cursor: pointer;" onclick="textarea_increase('cf_return_ip', 5);"><img src="<?=$g4['path']?>/adm/img/btn_down2.gif"></span></td>
		<td width=50% align=right><? if ($write_min || $write_max) { ?><span id=char_count></span>글자<?}?></td>
	</tr>
	</table>
	<textarea class=ed name='cf_return_ip' id='cf_return_ip' rows='8' style='width:99%;'><?=$config['cf_return_ip']?></textarea>
	<div style="padding-top:4px; line-height:140%;">
	※ 1라인에 IP|접속도메인|리턴URL 을 입력 하십시오. <u>(※ 3번째 인수인 리턴URL 은 http:// 포함하여 풀경로 입력)</u><BR>
	라인의 구분은 엔터이고, 3개조합의 구분은 | 입니다. (IP는 블럭으로도 가능합니다. 123.123.+)<BR>
	<b>예1) : 3개인수가 모두 있는 경우</b> 123.123.123.1 의 IP 가 OKFLEX.CO.KR 로 접속한 경우, OKFLEX.COM 으로 리디렉션 시키고자 할 때.<BR>
	123.123.123.1|OKFLEX.CO.KR|HTTP://OKFLEX.COM(또는 HTTP://WWW.OKFLEX.COM)<br>
	<b>예2) : IP만 있는 경우</b> 123.123.+ (이 경우는 IP만 체크하여 아래 기본리턴 URL 로 리디렉션 합니다.)<BR>
	<B>예3) : 접속도메인만 있는 경우</B> 123.123.+|OKFLEX.CO.KR (이 경우는 접속도메인을 체크하여 아래 기본리턴 URL 로 리디렉션 합니다.)<BR>
	<B>예4) : 접속도메인은 없고, 리턴URL 만 있는 경우</B> 123.123.+||HTTP://OKFLEX.COM (이 경우는 IP를 체크하여 리턴URL 로 리디렉션 합니다.(아래 기본리턴 URL 과는 별도로 동작합니다. <u>주의할점은 인수구분기호(|) 가 두개(||) 가 들어갑니다.</u>)
	</div>
	</TD>
</tr>
<tr>
	<td>기본리턴 URL</td>
	<td colspan=3><input type='text' class=ed name='cf_return_url' value='<?=$config['cf_return_url']?>' size=60> <?=help("※ 위 리턴 URL IP 에서 리턴시킬 URL 이 없는 경우 기본 리턴 URL 로 사용됩니다.")?></td>
</tr>
