...
function get_member_level_select($name, $start_id=0, $end_id=10, $selected='', $event='')
{
    ...
    for ($i=$start_id; $i<=$end_id; $i++)
    {
        ...
        if ($i == $selected)
            $str .= " selected";
        //$str .= ">$i</option>";
		$str .= ">".($GLOBALS['mb_level_str'][$i] != '' ? $GLOBALS['mb_level_str'][$i]."&nbsp;[".$i."]" : $i)."</option>";
    }
    $str .= "</select>";
    return $str;
}
...
// 텍스트에리어 늘리기, 줄이기
// 김선용 200805 : 확장
function textarea_size($fld, $size=10)
{
    global $g4;
...
// 작업아이콘 출력
function icon($act, $link="", $target="_parent" , $attr = '')
{
    global $g4;
    ...
    if ($link)
        ...
        $s = "<a href=\"$link\"".($attr ? " ".$attr:"").">$icon</a>";
    else
        $s = $icon;
    return $s;
}
...
/* YYYY-mm-dd 에서 1초 빼기 => 전날 23:59:59초 리턴 2015-01-22 홍민기 */
function second_minus($date){
	...
	$yesterday = date('Y-m-d H:i:s',mktime($hour,$min,$sec-1,$month,$day,$year));
	return $yesterday;
}
...
// 김선용 201107 : BUI IP 접근제한
if($config['cf_bui_ip_access_sw'])
{
	$ip_chk = sql_fetch("select bi_pid from {$g4['yc4_bui_ip_table']} where bi_access_ip='{$_SERVER['REMOTE_ADDR']}' ");
	if(!$ip_chk['bi_pid']) die();
}
