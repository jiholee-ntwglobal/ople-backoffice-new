...
$sql_common = "
	...
	where a.it_discontinued=0 and a.it_stock_qty < 1
";
...
<fieldset style="padding:5px; width:100%;">
	<legend>안내</legend>
	※ <B>품절상품의 재고를 1 이상으로 변경시, '판매 체크, 단종 체크해제 경우'</B> 해당 상품에 SMS 통보신청을 한 회원들에게 입고 SMS 를 발송합니다.
</fieldset>
...
<form name=fitemcardonly method=post action="./item_soldout_listupdate.php" style="margin:0;">
<input type=hidden name=sort1      value="<? echo $sort1 ?>">
<input type=hidden name=sort2      value="<? echo $sort2 ?>">
...
<tr>
	...
	<td class=yalign_head><label>입고처리<input type="checkbox" id="chk_on" /></label></TD>
	<td class=yalign_head><a href='<? echo title_sort("it_use") . "&$qstr1"; ?>'><span style="color:FFFFCC;">판매</span></a> | 단종</td>
...
<tr bgcolor="#FFFFFF" onmouseover="this.style.backgroundColor='#c9c9c9';" onmouseout="this.style.backgroundColor='#FFFFFF';" align=center>
	...
	<td>
		<label><input type="checkbox" name="it_stock_qty_on[]" value="1" onclick="if(this.checked) get_id('it_stock_qty[<?=$i?>]').value='99999'; else get_id('it_stock_qty[<?=$i?>]').value=get_id('it_stock_qty[<?=$i?>]').defaultValue;" />입고처리</label>
		<input type=text size=6 name='it_stock_qty[]' id='it_stock_qty[<?=$i?>]' value='<?=$row['it_stock_qty']?>'>
	</td>
	<td>
		<input type=checkbox style='background-color:#DBEEFB' name='it_use[<?=$i?>]' value='1' <?=($row[it_use] ? "checked" : "");?> />&nbsp;&nbsp;|&nbsp;
		<input type=checkbox style='background-color:gray' name='it_discontinued[<?=$i?>]' value='1' title="체크시 이 상품을 단종으로 처리합니다." />
	</td>
	<td><?=$s_mod?>&nbsp;<?=$s_del?></td>
</tr>
<?}?>
...
$(document).ready(function() {
	$('#chk_on').click(function() {
		if($('#chk_on').is(':checked')){
			$("input[name='it_stock_qty_on[]']").attr('checked', true);
			$("input[name='it_stock_qty[]']").val('99999');
		}else{
			$("input[name='it_stock_qty_on[]']").attr('checked', false);
			$("input[name='it_stock_qty[]']").each(function(){
				// 김선용 201305 : jquery : 라이브러리 버전변경으로 속성함수 변경으로 인해 재처리
