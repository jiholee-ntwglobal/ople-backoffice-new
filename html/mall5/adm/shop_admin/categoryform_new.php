...
$sql_common = " from $g4[yc4_category_table]_tmp ";
if ($is_admin != 'super')
...
if ($w == "")
{
    ...
    $sql = " select MAX(SUBSTRING(ca_id,$len2,2)) as max_subid from $g4[yc4_category_table]_tmp
    ...
    if ($ca_id) // 2단계이상 분류
    {
        $sql = " select * from $g4[yc4_category_table]_tmp where ca_id = '$ca_id' ";
        $ca = sql_fetch($sql);
...
<?=cheditor1('ca_head_html', '100%', '200');?>
<?=cheditor1('ca_tail_html', '100%', '200');?>

<form name=fcategoryform method=post action="./categoryformupdate_new.php" enctype="multipart/form-data" onsubmit='return fcategoryformcheck(this);' style="margin:0px;">
...
<tr class=ht>
    ...
    <? } else { ?>
        ...
        <? echo "<a href='./categoryform_new.php?ca_id=$ca_id&$qstr' title='하위분류 추가'><img src='$g4[admin_path]/img/icon_insert.gif' border=0 align=absmiddle></a>"; ?>
...
<tr class='ht'>
	<td>카테고리 출력</td>
	<td colspan='3'><input type="checkbox" name='ca_view' value='1' <?=($ca['ca_view'] == 1) ? "checked":"" ?> /></td>
</tr>
...
<tr class=ht>
    <td>META 타이틀</td>
    <td colspan=3><input type=text name=ca_meta_title value='<? echo $ca['ca_meta_title'] ?>' size=60 itemname="카테고리타이틀" class=ed></td>
</tr>

<tr class=ht>
    <td>META 키워드</td>
    <td colspan=3><input type=text name=ca_meta_keyword value='<? echo $ca['ca_meta_keyword'] ?>' size=60 itemname="메타키워드" class=ed></td>
</tr>
<tr class=ht>
    <td class=ht>META H1</td>
    <td colspan=3><input type=text name=ca_meta_h1 value='<? echo $ca['ca_meta_h1'] ?>' size=60 itemname="h1_text" class=ed></td>
</tr>
<tr class=ht>
    <td>META 설명</td>
    <td colspan=3><input type=text name=ca_meta_description value='<? echo $ca['ca_meta_description'] ?>' size=60 itemname="메타설명" class=ed></td>
</tr>
<!-- // 김선용 201306 : 상품상세페이지 메타처리 -->
<tr class=ht>
    <td>상품페이지 META 타이틀</td>
    <td colspan=3><input type=text name=ca_it_title value='<? echo $ca['ca_it_title'] ?>' size=60 itemname="상품페이지타이틀" class=ed></td>
</tr>
<tr class=ht>
    <td>상품페이지 META 키워드</td>
    <td colspan=3><input type=text name=ca_it_keyword value='<? echo $ca['ca_it_keyword'] ?>' size=60 itemname="상품페이지설명" class=ed></td>
</tr>
<tr class=ht>
    <td>상품페이지 META H1</td>
    <td colspan=3><input type=text name=ca_it_meta_h1 value='<? echo $ca['ca_it_meta_h1'] ?>' size=60 itemname="h1_text" class=ed></td>
</tr>
<tr class=ht>
    <td>상품페이지 META 설명</td>
    <td colspan=3><input type=text name=ca_it_description value='<? echo $ca['ca_it_description'] ?>' size=60 itemname="상품페이지설명" class=ed></td>
</tr>
...
<tr class=ht>
	...
	<!-- 김선용 200805 : 분류 출력순서 임의 설정 -->
	<td>출력순서</td>
	<td><input type="text" name="ca_order_print" id="ca_order_print" size=6 class="ed" value="<?=$ca['ca_order_print']?>">&nbsp;<?=help("음수일수록 우선순위를 가집니다. 최대가능 수는 -65536 ~ 65535 까지 입니다.")?></td>
</tr>
<tr class=ht>
    <td>출력스킨</td>
    <td colspan=3>
...
<tr class=ht>
    ...
    <td colspan=3 align=right style='padding-top:7px; padding-bottom:7px;'><?=cheditor2('fcategoryform', 'ca_head_html', stripslashes($ca[ca_head_html]));?></td>
</tr>
<tr class=ht>
    <td>하단 내용 <?=help("상품리스트 페이지 하단에 출력하는 HTML 내용입니다.", -150);?></td>
    <td colspan=3 align=right style='padding-top:7px; padding-bottom:7px;'><?=cheditor2('fcategoryform', 'ca_tail_html', stripslashes($ca[ca_tail_html]));?></td>
</tr>
<tr><td colspan=4 height=1 bgcolor=#CCCCCC></td></tr>
</table>
...
<p align=center>
    ...
    <input type=button class=btn1 accesskey='l' value='  목  록  ' onclick="document.location.href='./categorylist_new.php?<?=$qstr?>';">
</form>
...
function codedupcheck(id)
{
    ...
    window.open("./codedupcheck_new.php?ca_id="+id+'&frmname=fcategoryform', "hiddenframe");
}
