...
//전체리스트
$count_row = sql_fetch("select count(c.ct_id) as cnt
...
$que = sql_query($a=" select n.od_id, n.settlement_date, o.od_time, i.it_name, o.on_uid, TRUNCATE(o.od_send_cost/o.exchange_rate,2) as od_send_cost, 
                  TRUNCATE(o.od_receipt_point/o.exchange_rate,2) as od_receipt_point, 
                    ...
                    and (o.od_pay_time != '' and o.od_pay_time is not null)
                    $where
                    ORDER BY n.od_id desc, n.create_date desc, FIELD(c.ct_status, '품절', '반품', '배송','준비','완료')
...
if($_GET['excel']=="download"){
    ...
    $objPHPExcelWrite->setCellValue('A1','주문번호')
        ...
        ->setCellValue('E1','주문일시')
        ->setCellValue('F1','결제일시')
        ->setCellValue('G1','취소일시')
        ->setCellValue('H1','수취일시')
        ->setCellValue('I1','상품금액')
        ->setCellValue('J1','주문금액')
        ->setCellValue('K1','할인금액(포인트)')
        ->setCellValue('L1','배송료')
        ->setCellValue('M1','과세')
        ->setCellValue('N1','최종결제금액');
    ...
    while($value= sql_fetch_array($que)){
        ...
        $objPHPExcelWrite->getCell('C'.$line)->setValueExplicit($value['it_id'], PHPExcel_Cell_DataType::TYPE_STRING);
        $objPHPExcelWrite->setCellValue('D'.$line,$value['ct_status'])
            ->setCellValue('E'.$line,$value['od_time'])
            ->setCellValue('F'.$line,$value['pay_date'])
            ->setCellValue('G'.$line,$value['cancel_date'])
            ->setCellValue('H'.$line,$value['settlement_date'])
            ->setCellValue('I'.$line,$value['item_amount'])
            ->setCellValue('J'.$line,$order_amount)
            ->setCellValue('K'.$line,$value['od_receipt_point'])
            ->setCellValue('L'.$line,$value['od_send_cost'])
            ->setCellValue('M'.$line,0)
            ->setCellValue('N'.$line,$total_amount);
    ...
    <div class="row">
        ...
        <div class="col-lg-2">
            <?php if (count($list_value)>0) { ?>
                <button class="btn btn-success" type="button" onclick="location.href='./naver_cps_order.php?excel=download<?php echo $dstr?>'">엑셀다운로드</button>
            <?php } ?>
        </div>
    </div>
</form>
            ...
            if (count($list_value)>0) {
                foreach ($list_value as $value) {
                   ...
                   $total_amount = round((($value['total_amount']  * 100)- ($cancel_amount[$value['od_id']]*100))/100,2);
                    ?>
                    <tr>
                        <td><?php echo $value['od_id'] ?></td>
                        <td align="left"><?php echo get_item_name($value['it_name'],'list');?></td>
                        <td><a href="https://www.ople.com/mall5/shop/item.php?it_id=<?php echo $value['it_id']?>" target="_blank"><?php echo $value['it_id']; ?></a></td>
                        <td><?php echo $value['ct_status']?></td>
