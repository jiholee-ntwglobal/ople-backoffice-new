...
<form name=fcategorylist method='post' action='./categorylistupdate_new.php' autocomplete='off' style="margin:0px;">
...
<tr align=center class=ht>
    ...
    <td width=120>
        <? 
        if ($is_admin == 'super')
            echo "<a href='./categoryform_new.php'><img src='$g4[admin_path]/img/icon_insert.gif' border=0 title='1단계분류 추가'></a>";
        else
            echo "&nbsp;";
        ?>
    </td>
...
for ($i=0; $row=sql_fetch_array($result); $i++) 
{
    ...
    $s_add = icon("추가", "./categoryform_new.php?ca_id=$row[ca_id]&$qstr");
    $s_upd = icon("수정", "./categoryform_new.php?w=u&ca_id=$row[ca_id]&$qstr");
    $s_vie = icon("보기", "$g4[shop_path]/list.php?ca_id=$row[ca_id]");

    if ($is_admin == 'super')
        $s_del = icon("삭제", "javascript:del('./categoryformupdate_new.php?w=d&ca_id=$row[ca_id]&$qstr');");
	...
	*/
	...
	$sql1 = "
		select count(*) as cnt from yc4_category_item_tmp where ca_id like '".$row['ca_id']."%'
	";
    $row1 = sql_fetch($sql1);
