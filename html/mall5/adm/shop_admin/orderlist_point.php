...
/*
----------------------------------------------------------------------
file name	 : orderlist_point.php
comment		 : 전액 포인트 결제 주문서 리스트
date		 : 2014-12-03
...
if($_GET['sel_field']){
    ...
    if($search !=''){
        if($_GET['sel_field'] =='od_name'){
            $sql_search .= " and ".$_GET['sel_field']." like '%".trim($search)."%' ";
        }else{
            $sql_search .= " and ".$_GET['sel_field']." = '".trim($search)."' ";
        }
    }
...
$sql_common = "
	...
	where
		...
		a.od_temp_point > 0
		and
		a.od_temp_point = a.od_receipt_point
		".$sql_search."
";

$total_count = sql_fetch("select count(distinct a.on_uid) as cnt ".$sql_common);
...
for ($i=0; $row=mysql_fetch_array($result); $i++)
{
    ...
    <tr class='list$list ht'>
        ...
        <td align=center>$s_mod $s_del</a><input type='button' value='준비로 변경' onclick=\"ct_status_change('".$row['on_uid']."');\"/></td>
    </tr>";
    ...
    // 상품개별출력
    $sql2 = " select c.it_id,c.it_name,
                     b.ct_status,b.ct_amount,b.ct_point,b.ct_qty
...
<form action="orderlist_point_update.php" method='post' name='update_frm'>
	<input type='hidden' name="on_uid" />
	<input type="hidden" name='sel_field' value='<?=$_GET['sel_field'];?>'/>
...
function ct_status_change(on_uid){
	if(!confirm('해당 주문건을 준비로 변경하시겠습니까?')){
		return false;
	}
	update_frm.on_uid.value = on_uid;
	update_frm.submit();
}
</script>
