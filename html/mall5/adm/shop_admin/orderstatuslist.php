...
if ($search != "") { //곽범석 쿼리 개선
    if ($sel_field == "c.ca_id" || $sel_field == "od_name") {
    	$sql_search .= " $where $sel_field like '$search%' ";
        $where = " and ";
    } else if ($sel_field != "") {
    	$sql_search .= " $where $sel_field = '$search' ";
        $where = " and ";
    }
...
// 김선용 201005 :  : 레코드 누적에 따른 카운트 지연으로 카운트쿼리 변경
// 테이블의 전체 레코드수만 얻음
if($search != '')
	$sql = " select count(a.od_id) as cnt " . $sql_common;
else
	$sql = " select count(a.od_id) as cnt from $g4[yc4_order_table] a left join $g4[yc4_cart_table] b on (a.on_uid = b.on_uid)";
