...
# 검색 처리 #
if($_GET['st_dt']){
	$search .= (($search) ? ' and ':'')." a.st_dt >= '".$_GET['st_dt']."'";
}
if($_GET['en_dt']){
	$search .= (($search) ? ' and ':'')." a.en_dt <= '".$_GET['en_dt']."'";
}
if($_GET['value']){
	$search .= (($search) ? ' and ':'')." ".$_GET['key']." like '%".$_GET['value']."%'";
}
...
switch($_GET['fg']){
    case 'Y' : $search .= ($search ? " and ":"")." date_format(now(),'%Y%m%d') between a.st_dt and a.en_dt"; break;
    case 'N' : $search .= ($search ? " and ":"")." date_format(now(),'%Y%m%d') not between a.st_dt and a.en_dt"; break;
}
...
$qry = "
	...
	from
		...
		left join
		yc4_item d on a.l_it_id = d.it_id
	".$search."
	order by
		a.create_dt desc
";
...
$total_couint = (!$total_couint) ? 0 : $total_couint;
...
$sql = sql_query($qry." limit ".$from_record.", ".$config['cf_page_rows']);
...
while($oneday_item = mysql_fetch_assoc($sql)){
	$list_tr .= "
		<tr class='list$list center ht'>
			...
			<td rowspan='2'>".icon("수정", "./oneday_write.php?uid=".$oneday_item['uid']).'&nbsp;'.icon("삭제", $_SERVER['PHP_SELF'].'?mode=del&uid='.$oneday_item['uid'])."</td>
		</tr>
		<tr class='list$list center ht'>
...
<table cellpadding='0' cellspacing='0' width='100%'>
	<tr>
		<td height='2' bgcolor='#0E87F9' colspan='9'></td>
	</tr>
	<tr align='center' class='ht'>
		...
		<td>시작일</td>
		<td>판매금액</td>
		<td rowspan='2'><?=icon("입력", "./oneday_write.php");?></td>
	</tr>
