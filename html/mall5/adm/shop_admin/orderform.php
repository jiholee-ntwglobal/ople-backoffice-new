...
// 준비상태 변경가능 아이디 목록 2018-02-02 강경인
$ready_member	= array('dev','okyo','naya2410','bmyyko84');
...
//------------------------------------------------------------------------------
// slow query 개선 주석 처리 - 20171023 rsmaker@ntwglobal.com
...
$cd_history = sql_fetch("select * from ".$g4['yc4_card_history_table']." where od_id = '".$od_id."'");
...
$upc_in = '';
while($row = sql_fetch_array($item_sql)){
...
}elseif($od['ihappy_fg']=='k'){
    $order_msg = "국민카드 아이해피몰 주문건 입니다.";
}elseif($od['opk_fg'] == 'Y'){
...
if($shipping_chk){
    ...
    $invoice_chk = $db->ntics_db->query("
            SELECT a.CJNUM, count (a.CJNUM) cnt
              FROM ntshipping.dbo.ns_invoice a INNER JOIN ntshipping.dbo.ns_invoice b ON a.CJNUM = b.CJNUM
              WHERE a.ordercode = 'k{$od_id}'
            GROUP BY a.CJNUM
    ")->fetchObject();

    $invoice_cjnum_data=$invoice_chk->CJNUM;

    $invoice_chk = $invoice_chk->cnt;
...
//    $ct_amount_usd = $row['ct_amount_usd'] ? $row['ct_amount_usd'] : usd_convert($row['ct_amount'],$od['exchange_rate']);
	...
	# 매핑된 상품인지 체크 #
	$ntics_info = '';
	$mapping_chk = sql_query("select upc,qty from ople_mapping where it_id = '".$row['it_id']."'");
	while($row2 = sql_fetch_array($mapping_chk)){
		$ntics_info = "
    ...
    echo "
    <tr>
        <td title='$row[ct_id]'><input type='hidden' name='bf_ct_status[$i]' value='".$row['ct_status']."'/><input type=hidden name=ct_id[$i] value='$row[ct_id]'><input type=checkbox id='ct_chk_{$i}' name='ct_chk[{$i}]' value='1'></td>
        ...
        <td>
            ".($row['it_health_cnt'] * $row['ct_qty'])."<br/>".$row['it_health_cnt']."<br/><a href='$g4[shop_admin_path]/item_health_cnt.php?it_id=".$row['it_id']."' target='_blank'>수정</a>
        </td>";
    echo "</tr>";
	echo $ntics_info;
    $tot_health_cnt += $row['it_health_cnt'] * $row['ct_qty'];

    $t_amount_usd	+= $ct_amount_usd * $row['ct_qty'];
...
<tr class='ADlist_resultBox'>
    <td colspan=3 style='text-align:left;padding-left:20px;'>
        ...
        <?php if(in_array($_SESSION['ss_mb_id'], $ready_member)){?>
    ...
    <td colspan=3>
		...
		?>
		<b>주문합계 : <? echo number_format($t_ct_amount['합계']+$edit_ct_amount); ?>원($ <?php echo number_format($t_amount_usd,2); ?>)</B>
...
//$amount['미수u'] = round((($amount['정상'] - $od[od_dc_amount] + $od[od_refund_amount]) - ($amount['입금'] - $od[od_cancel_card]))/$od[exchange_rate],2);
...
<tr><td colspan=10 style='color:#FF6600;text-align:right;border-top:solid 1px #666;'><strong>미수금 : <?php echo display_amount($amount['미수']); ?>($<?php echo $t_ct_amount['취소Usd']; ?>)</strong></td></tr>
</tbody>
</table>
<?php
echo "<b>";
    ...
    <td width=50% valign=top align=center>
		...
		<tr>
			<td colspan=3 style='text-align:center;padding:20px 0;'>
				...
				<input type=button class=btn1 value='  목  록  ' onclick="document.location.href='./orderlist.php?<?php echo $qstr?>';">
			</td>
		</tr>
</table>
</form>
...
<tr>
	...
	<?}else if($od['od_ship'] == '1'){?>
		<?
		$os_sql = sql_query("select * from {$g4['yc4_os_table']} where on_uid='{$od['on_uid']}' and od_id='$od_id' order by os_pid ");
			...
			<tr align='left'>
				...
				<td>
					<select name='os_dl_id[]'>
						...
						<?
						$sql2 = "select * from $g4[yc4_delivery_table] order by dl_order desc, dl_id desc ";
    ...
    <div class="shipping_ajax_data"></div>
...
<p style='padding:20px 0;text-align:center;'>
    ...
    <input type=button class=btn1 value='  목  록  ' accesskey='l' onclick="document.location.href='./orderlist.php?<?php echo $qstr?>';">&nbsp;
...
$(document).ready(function () {
    $.ajax({
        type : "POST"
        , dataType : "html"
        , url : "./shipping_ajax_data.php"
        , data : {
            "invoice_cjnum_data" : '<?php echo trim($invoice_cjnum_data) ;?>'
            , "invoice_chk" : '<?php echo trim($invoice_chk) ;?>'
            , "dl_company" : '<?php echo trim($dl[dl_company]) ;?>'
            , "od_invoice_time" : '<?php echo trim($od[od_invoice_time]) ;?>'
        }
        , success : function (data) {
            $('.shipping_ajax_data').html(data);
        }
    });
});
