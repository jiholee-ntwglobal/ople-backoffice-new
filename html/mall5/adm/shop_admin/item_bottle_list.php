...
<form name=fitemlist method=post action="./item_bottle_listupdate.php" style="margin:0;">
<input type=hidden name=sort1      value="<? echo $sort1 ?>">
<input type=hidden name=sort2      value="<? echo $sort2 ?>">
...
<tr>
	...
	<td class=yalign_head><a href='<? echo title_sort("it_bottle_count") . "&$qstr1"; ?>'><span style="color:FFFFCC;">병수량</span></a></td>
...
for($i=0; $row=sql_fetch_array($result); $i++)
{
	...
	$s_mod = icon("수정", "./itemform.php?w=u&it_id=$row[it_id]&return_url=item_bottle_list&$qstr");
	$s_del = icon("삭제", "javascript:del('./itemformupdate.php?w=d&return_url=item_bottle_list&it_id={$row['it_id']}&$qstr');");
...
<tr bgcolor="#FFFFFF" onmouseover="this.style.backgroundColor='#c9c9c9';" onmouseout="this.style.backgroundColor='#FFFFFF';" align=center>
	...
	<td><input type=text size=4 name='it_bottle_count[<?=$i?>]' value='<?=$row['it_bottle_count']?>'></td>
