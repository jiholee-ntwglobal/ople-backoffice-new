...
?>
    <style>
        ...
        .kb {
            background-color: rgba(255, 160, 122, 0.46);
        }
        .ihappy {
            background-color: rgba(255, 141, 255, 0.25);
        }
    </style>
...
<?php } ?>
    ...
    <ul class="list_tab">
        ...
        <li class="<?php echo $_GET['od_fg'] == 'k' ? "active" : "" ?>"><a href="<?php echo $_SERVER['PHP_SELF']; ?>?od_fg=k&<?php echo $qstr2; ?>">KB몰 주문서</a></li>
    ...
    <table width=100% cellpadding=0 cellspacing=0>
        ...
        for ($i = 0; $row = mysql_fetch_array($result); $i++) {
            ...
            if ($row['ihappy_fg']=='Y') {
                ...
                $ihappy_msg = "<b style='color:#ff0000;'>KB</b>";
            }else{
                $ihappy_msg ='';
            }
    ...
    <tr class='list$list $mobile_class ht'>
        <td align=center title='주문일시 : $row[od_time]'>" . $ihappy_msg . "<a href='http://209.216.56.107/mall5/shop/orderinquiryview.php?od_id=$row[od_id]&on_uid=$row[on_uid]' target='_blank'>$row[od_id]</a>{$od_id_ip}</td>
