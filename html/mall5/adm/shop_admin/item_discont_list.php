...
<form name=fitemcardonly method=post action="./item_discont_listupdate.php" style="margin:0;">
<input type=hidden name=sort1      value="<? echo $sort1 ?>">
<input type=hidden name=sort2      value="<? echo $sort2 ?>">
...
<tr>
	...
	<td class=yalign_head><a href='<? echo title_sort("it_use") . "&$qstr1"; ?>'><span style="color:FFFFCC;">판매</span></a> | <label>단종해제<input type="checkbox" id="chk_on" /></label></td>
...
<tr bgcolor="#FFFFFF" onmouseover="this.style.backgroundColor='#c9c9c9';" onmouseout="this.style.backgroundColor='#FFFFFF';" align=center>
	...
	<td>
		...
		<input type=checkbox style='background-color:gray' name='it_discontinued[<?=$i?>]' class="js_count" value='1' title="체크시 이 상품을 단종에서 해제합니다." />
	</td>
	<td><?=$s_mod?>&nbsp;<?=$s_del?></td>
</tr>
<?}?>
...
$(document).ready(function() {
	$('#chk_on').click(function() {
		...
		if($('#chk_on').is(':checked')){
			$(".js_count").attr('checked', 'checked');
		}else{
			$(".js_count").attr('checked', '');
		}
		*/
		if($('#chk_on').is(':checked')){
			$(".js_count").prop('checked', true);
		}else{
			$(".js_count").prop('checked', false);
		}
