...
if($yy){
	if($yy-5 < 0 || $yy - date('y', strtotime('-3 year')) > 0){
		$yy = '';
	} else {
		$g4['yc4_order_table'] = 'yc4_order_'.str_pad($yy,2,'0',STR_PAD_LEFT);
		$g4['yc4_cart_table'] = 'yc4_cart_'.str_pad($yy,2,'0',STR_PAD_LEFT);
	}
}
...
$qstr1 = "sel_field=$sel_field&search=$search&save_search=$search&yy=$yy";
...
<tr>
	...
	<td width=10%>
	<select name="yy" onchange="location.href='<?=$_SERVER[PHP_SELF]?>?yy='+this.value;">
	<option value="">최신 주문정보(<?php echo date('Y',strtotime('-2 year')); ?>년~현재)</option>
	<?php
	for($k=date('y',strtotime('-3 year'));$k>4;$k--){
		$selected = $yy==$k ? 'selected' : '';
		echo "<option value='$k' $selected>20".str_pad($k,2,'0',STR_PAD_LEFT)."년 주문서</option>";
	}
	?>
	</select>
	</td>
...
<ul class="list_tab">
    <li class="<?php echo !$_GET['od_fg'] ? "active":""?>"><a href="<?php echo $_SERVER['PHP_SELF'];?>?<?php echo $qstr2;?>">오플주문서</a></li>
...
for ($i=0; $row=mysql_fetch_array($result); $i++)
{
    ...
    <tr class='list$list $mobile_class ht'>
        <td align=center title='주문일시 : $row[od_time]'>".$ihappy_msg."<a href='$g4[shop_path]/orderinquiryview.php?od_id=$row[od_id]&on_uid=$row[on_uid]'>$row[od_id]</a>{$od_id_ip}</td>
