...
$sql = " update $g4[yc4_default_table]
            set de_admin_company_owner      = '$de_admin_company_owner',
				...
				/* 김선용 200801 */
				de_conv_pay					= trim('$de_conv_pay'),
				de_authorize_id				= trim('$de_authorize_id'),
				de_authorize_key			= trim('$de_authorize_key'),
				de_post_mail_addr			= trim('$de_post_mail_addr'),
				de_receive_mail_addr		= trim('$de_receive_mail_addr'),
				/* // 김선용 2014.03 : */
				de_kcp_card_use		= '$de_kcp_card_use',
				de_kcp_escrow_use	= '$de_kcp_escrow_use',
				de_kcp_escrow_point	= trim('$de_kcp_escrow_point'),
				de_ihappy_sale_percent = '$de_ihappy_sale_percent'
                ";
if ($de_icode_pw)
	$sql .= "  , de_icode_pw = '$de_icode_pw' ";
sql_query($sql);

if($_POST['bf_de_conv_pay'] != $_POST['de_conv_pay']){ // 환율 변동이 있을 경우 가격 업데이트 처리
    sql_query("update yc4_item set it_amount=ceil(it_amount_usd*{$_POST['de_conv_pay']}) where it_amount_usd>0");
    sql_query("update yc4_item set it_cust_amount=ceil(it_cust_amount_usd*{$_POST['de_conv_pay']}) where it_cust_amount_usd>0");
    sql_query("insert into exchange_rate_history (dt,pay) VALUES (now(),'{$_POST['de_conv_pay']}')");
...
file_get_contents('http://ople.com/mall5/cron/config_data_cache.php');
