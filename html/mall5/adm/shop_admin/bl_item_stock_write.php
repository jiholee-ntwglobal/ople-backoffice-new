...
if($_POST['mode'] == 'insert'){
	...
	# 이미 등록된 상품코드인지 체크
	$chk2 = sql_fetch("select count(*) as cnt from yc4_bl_event_item_stock where it_id = '".$it_id."'");
	if($chk2['cnt']>0){
		alert('이미 등록된 상품입니다.');
		exit;
	}
	...
	$auto_sold_out = $_POST['auto_sold_out'] ? $_POST['auto_sold_out'] : 'n';



	$sql = "
		insert into
			yc4_bl_event_item_stock
		(
			it_id,qty,ch_amount,create_dt,comment,auto_sold_out
		)values(
			'".$it_id."','".(int)$_POST['qty']."','".(int)$_POST['ch_amount']."','".date('Y-m-d H:i:s')."','".$comment."','$auto_sold_out'
		)
	";
	...
	if(!sql_query($sql)){
		...
		alert('저장되었습니다.','bl_item_stock.php');
	}
...
if($_GET['mode'] == 'delete'){
	...
	$sql = "delete from yc4_bl_event_item_stock where uid = '".(int)$_GET['uid']."'";
	...
	if(!sql_query($sql)){
		...
		alert('삭제되었습니다.','bl_item_stock.php');
	}
...
<form action="<?=$_SERVER['PHP_SELF'];?>" method='post'>
	...
	<table width='100%'>
		...
		<tr>
			...
			<td>수량</td>
			<td><input type="text" name='qty' value='<?=$it['qty']?>'/>&nbsp;판매수량 : <?=$it['sell_cnt']?></td>
		</tr>
		...
		<tr>
			<td>사용</td>
			<td><input type="checkbox" name='auto_sold_out' value='y' <?=$it['auto_sold_out'] == 'y' ? "checked":""?>/></td>
		</tr>
	</table>
	<p align='center'><input type="submit" value=' 저장 ' /><?=$del_btn;?> <input type="button" value=' 목록 ' onclick="location.href='bl_item_stock.php'" /></p>
...
function it_delete(){
	...
	location.href="<?php echo $_SERVER['PHP_SELF'];?>?mode=delete&uid=<?php echo $it['uid']; ?>";
	return false;
}
</script>
