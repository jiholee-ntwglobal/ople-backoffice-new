...
$sql_common = " ca_skin         = '$ca_skin',
                ...
                ca_mb_id        = '$ca_mb_id',
				/* 김선용 200805 */
				ca_order_print	= '$ca_order_print',

				/* // 김선용 201306 : seo */
				ca_meta_title		= '$ca_meta_title',
				ca_meta_keyword		= '$ca_meta_keyword',
				ca_meta_h1			= '$ca_meta_h1',
				ca_meta_description	= '$ca_meta_description',
				ca_it_title			= '$ca_it_title',
				ca_it_keyword		= '$ca_it_keyword',
				ca_it_meta_h1		= '$ca_it_meta_h1',
				ca_it_description	= '$ca_it_description',
				ca_view				= '".$ca_view."'
				";
...
if ($w == "")
{
    ...
    $sql = " insert $g4[yc4_category_table]_tmp
                set ca_id   = '$ca_id',
                    ca_name = '$ca_name',
                    $sql_common ";
    sql_query($sql);
...
{
    $sql = " update $g4[yc4_category_table]_tmp
                set ca_name = '$ca_name',
                    $sql_common
              where ca_id = '$ca_id' ";
    sql_query($sql);
...
else if ($w == "d")
{
    ...
    $sql = " select COUNT(*) as cnt from $g4[yc4_category_table]_tmp
              where SUBSTRING(ca_id,1,$len) = '$ca_id'
                and ca_id <> '$ca_id' ";
    $row = sql_fetch($sql);
    if ($row[cnt] > 0)
    ...
    $str = $comma = "";
    $sql = " select it_id from $g4[yc4_item_table]_tmp where ca_id = '$ca_id' ";
    ...
    // 분류 삭제
    $sql = " delete from $g4[yc4_category_table]_tmp where ca_id = '$ca_id' ";
    sql_query($sql);
}
...
if ($w == "" || $w == "u")
{
    ...
    //goto_url("./categoryform_new.php?w=u&ca_id=$ca_id&$qstr");
	goto_url("./categorylist_new.php?$qstr");
} else {
    goto_url("./categorylist_new.php?$qstr");
}
?>
