...
if ($_GET['od_fg'] == 'opk') {
    ...
    $sql_search .= $where . " a.ihappy_fg = 'k'";
    $where = "and";
} elseif ($_GET['od_fg'] == 'Y') {
    $sql_search .= $where . " a.ihappy_fg = 'Y'";
    $where = "and";
} elseif ($_GET['od_fg'] == 'h') {
    $sql_search .= $where . " a.ihappy_fg = 'h'";
    $where = "and";
}
...
if ($search != "") {
    if ($sel_field != "") {
        ...
        if ($sel_field == 'receiptamount1') { // 이하
            ...
            $sql_search .= " $where a.$sel_field like '$search%' "; // 슬로우 쿼리 개선 alias 추가 2017-10-23 rsmaker@ntwglobal.com
        } else {
            //$sql_search .= " $where  a.$sel_field like '%$search%' ";
            $sql_search .= " $where  a.$sel_field = '$search' "; // 슬로우 쿼리 개선 like 변경 2017-10-23 rsmaker@ntwglobal.com
        }
...
if ($sort1 == "") $sort1 = "od_id"; // 슬로우 쿼리 개선 alias 추가 2017-10-23 rsmaker@ntwglobal.com
if ($sort2 == "") $sort2 = "desc";
...
*/
$sql = sql_fetch("select count(*) as cnt from {$g4['yc4_order_table']} a $sql_search");
$total_count = $sql['cnt'];
...
?>
    <style>
        ...
        .opk {
            background-color: rgba(154, 205, 50, 0.28);
        }
        .kb {
            background-color: rgba(255, 160, 122, 0.24);
        }
        .ihappy {
            background-color: rgba(255, 141, 255, 0.13);
        }
    </style>
...
<?php } ?>
    ...
    <ul class="list_tab">
        ...
        <li class="<?php echo $_GET['od_fg'] == 'k' ? "active" : "" ?>"><a href="<?php echo $_SERVER['PHP_SELF']; ?>?od_fg=k&<?php echo $qstr2; ?>">국민카드아이해피 주문서</a></li>
        <li class="<?php echo $_GET['od_fg'] == 'Y' ? "active" : "" ?>"><a href="<?php echo $_SERVER['PHP_SELF']; ?>?od_fg=Y&<?php echo $qstr2; ?>">아이해피몰 주문서</a></li>
        <li class="<?php echo $_GET['od_fg'] == 'h' ? "active" : "" ?>"><a href="<?php echo $_SERVER['PHP_SELF']; ?>?od_fg=h&<?php echo $qstr2; ?>">하나몰 주문서</a></li>
    ...
    <table width=100% cellpadding=0 cellspacing=0>
        ...
        for ($i = 0; $row = mysql_fetch_array($result); $i++) {
            ...
            if ($row['mobile_fg'] == 'Y') {
                ...
                $mobile_class = ' kb';
            } elseif ($row['ihappy_fg']=='Y') {
                $mobile_class = 'ihappy';
            }elseif ($row['ihappy_fg']=='h') {
                $mobile_class = 'hana';
            }
            $list = $i % 2;
            echo "
    <tr class='list$list $mobile_class ht'>
        <td align=center title='주문일시 : $row[od_time]'>" . $ihappy_msg . "<a href='http://66.209.90.21/mall5/shop/orderinquiryview.php?od_id=$row[od_id]&on_uid=$row[on_uid]' target='_blank'>$row[od_id]</a>{$od_id_ip}</td>
