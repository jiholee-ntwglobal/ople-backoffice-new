...
for ($i=0; $i<count($_POST[ca_id]); $i++) 
{
    if ($_POST[ca_mb_id][$i])
    {
        ...
        $row = sql_fetch($sql);
        if (!$row[mb_id])
            alert("\'{$_POST[ca_mb_id][$i]}\' 은(는) 존재하는 회원아이디가 아닙니다.", "./categorylist_new.php?page=$page&sort1=$sort1&sort2=$sort2");
    }
...
goto_url("./categorylist_new.php?page=$page&sort1=$sort1&sort2=$sort2");
?>
