...
<tr bgcolor=#ffffff class=ht>
    ...
    <td colspan=3 align=right>
		...
		?>
		<b>주문합계 : <? echo number_format($t_ct_amount[합계]+$edit_ct_amount); ?>원</B></td>
...
<tr align=center class=ht>
	...
	<td><?=nf($od['od_recommend_off_sale'])?></td>
...
<tr><td colspan=10 height=1 bgcolor=#CCCCCC></td></tr>
<tr><td colspan=10 align=right class=ht><b><font color=#FF6600><b>미수금 : <? echo display_amount($amount[미수]) ?></b></font></b></td></tr>
</table>
...
<input type="hidden" name="od_ship" value="<?=$od['od_ship']?>" /> <!-- // 김선용 201211 : -->
...
<tr>
    <td width=49% valign=top>
        ...
        <tr class=ht>
			...
			?>
            <td><input type=text name='od_send_cost' value='<?=$edit_send_cost?>' class=ed size=10 style='text-align:right;'>원
...
<tr>
    <td width=49% valign=top bgcolor=#ffffff>
		...
		*/
		...
		<tr class=ht>
			<td>주민번호</td>
			<td><?=substr($jumin,0,6)?>-<?=substr($jumin,6)?></td>
		</tr>
		-->
	...
	<?}else if($od['od_ship'] == '1'){?>
		...
		{
		?>
			<input type="hidden" name="os_pid[]" value="<?=$row['os_pid']?>" />
		   <tr class=ht>
				<td>보내는사람</td>
				<td><input type=text class=ed name='os_post_name[]' value='<?=$row['os_post_name']?>' required itemname='보내는사람'>&nbsp;&nbsp;<a href="javascript:;" onclick="view_ship_item('<?=$row['os_pid']?>');" title='배송상품보기'>배송상품보기</a></td>
			</tr>
		   <tr class=ht>
				<td>이름</td>
				<td><input type=text class=ed name='os_name[]' value='<?=$row['os_name']?>' required itemname='받으시는 분 이름'></td>
			</tr>
			<tr class=ht>
				<td>전화번호</td>
				<td><input type=text class=ed name='os_tel[]' value='<?=$row['os_tel']?>' required itemname='받으시는 분 전화번호'></td>
			</tr>
			<tr class=ht>
				<td>핸드폰</td>
				<td><input type=text class=ed name='os_hp[]' value='<?=$row['os_hp']?>'></td>
			</tr>
			<tr class=ht>
				<td>주소</td>
				<td>
					<input type=text class=ed name='os_zip1[]' size=4 readonly required itemname='우편번호 앞자리' value='<?=$row['os_zip1']?>'> -
					<input type=text class=ed name='os_zip2[]' size=4 readonly required itemname='우편번호 뒷자리' value='<?=$row['os_zip2']?>'>
					&nbsp;<a href="javascript:;" onclick="win_zip('frmorderform2', 'os_zip1[]', 'os_zip2[]', 'os_addr1[]', 'os_addr2[]', '<?=$k?>');"><img src="<?=$g4[shop_admin_path]?>/img/btn_zip_find.gif" border=0 align=absmiddle></a><br>
					<input type=text class=ed name='os_addr1[]' size=50 readonly required itemname='주소' value='<?=$row['os_addr1']?>'><br>
					<input type=text class=ed name='os_addr2[]' size=50 required itemname='상세주소' value='<?=$row['os_addr2']?>'></td>
			</tr>
			<? if ($default[de_hope_date_use]) { ?>
			<tr class=ht>
				<td>희망배송일</td>
			...
			<tr class=ht>
				<td>전하는 말씀</td>
				<td colspan=3><?=nl2br($row['os_memo'])?></td>
			</tr>
			<tr><td colspan=2 height=1 bgcolor=CCCCCC></td></tr>
			...
			<tr class=ht>
				<td>배송일시</td>
				<td>
					<input type=text class=ed name='os_invoice_time[]' id='os_invoice_time[<?=$k?>]' maxlength=19 value='<? echo is_null_time($row[os_invoice_time]) ? "" : $row[os_invoice_time]; ?>'>
					<label><input type=checkbox value="<? echo $g4['time_ymdhis']; ?>"
						onclick="if(this.checked) get_id('os_invoice_time[<?=$k?>]').value=this.value; else get_id('os_invoice_time[<?=$k?>]').value = get_id('os_invoice_time[<?=$k?>]').defaultValue;">현재 시간</label>
				</td>
			</tr>
			<tr class=ht>
				<td>메일발송</td>
			...
			<tr><td colspan=2 height=1 bgcolor=CCCCCC></td></tr>
			<tr><td colspan=2 height=1 bgcolor=blue></td></tr>
			<tr><td colspan=2 height=1 bgcolor=CCCCCC></td></tr>
		<?}?>
	<?}?>
...
function view_ship_item(os_pid)
{
	$.ajax({
		...
		data: { 's_type' : 'view', 'os_pid' : os_pid, 'tmp_on_uid' : '<?=$od['on_uid']?>' },
		cache: false,
		async: false,
