...
if($_POST['mode'] == 'insert'){
	...
	alert('저장이 완료되었습니다.',$g4['shop_admin_path'].'/station_cate_new.php?s_id='.$_POST['s_id']);
	exit;
}
...
if($_POST['mode'] == 'delete'){
	$sql = "
		delete from shop_category_tmp where uid = '".$_POST['uid']."'
	";
	...
	alert('삭제가 완료되었습니다.',$g4['shop_admin_path'].'/station_cate_new.php?s_id='.$_POST['s_id']);
	exit;
}

# 제품관 카테고리 데이터 로드 #
$station_cate = sql_fetch("
	select * from shop_category_tmp where uid = '".$_GET['uid']."' and s_id = '".$_GET['s_id']."' order by sort
");
...
$sql = sql_query("
	select
		a.ca_id,a.ca_name
	from
		".$g4['yc4_category_table']."_tmp a
	where
		length(a.ca_id) = 2
		and
		a.ca_id not in (select ca_id from shop_category_tmp)
...
if($station_cate['sort']){
	...
	$sort_qry = sql_fetch("select max(sort)+1 as sort from shop_category_tmp where s_id = '".$_GET['s_id']."'");
	$sort = $sort_qry['sort'];
}
...
<form action="<?=$_SERVER['PHP_SELF']?>" method='post'>
	...
	<p align='center'>
		...
		<input type="button" value=' 목 록 ' onclick="location.href='<?=$g4['shop_admin_path']?>/station_cate_new.php?s_id=<?=$_GET['s_id']?>'"/>
