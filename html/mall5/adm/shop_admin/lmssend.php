...
if($_POST['mode'] == 'send'){
    ...
    if(!$_POST['SUBJECT']){
        ...
        $SUBJECT = $default['de_admin_company_name'];
    }

    $insert_sql = "
      INSERT INTO yc4_lms
      (REQDATE ,CALLBACK ,SUBJECT, MSG ,send_code ,send_addtion_no ,mb_id ,create_dt) VALUES
      (".$send_date.",'".$callback_no."','".sql_safe_query($SUBJECT)."','".$contents."','".sql_safe_query($_POST['send_code'])."','".sql_safe_query($send_hp)."','".$member['mb_id']."',now())
    ";

    sql_query($insert_sql);


    alert('LMS 발송 등록이 완료되었습니다.',$_SERVER['PHP_SELF']);
...
?>
    ...
    <form action="<?php echo $_SERVER['PHP_SELF']; ?>" onsubmit="return sms_send();" method="post">
        ...
        <input type="hidden" name='send_code' value='goodday'/>


        <div>제목</div>
        <input type="text" name="SUBJECT">
        <div>내용</div>
        <textarea name="contents" id="" rows="30" ONKEYUP="byte_check(this,bytes)" style="width: 100%;"></textarea>
        <div>
            <span id="bytes">0 / 2,000 bytes</span>
        </div>
        <div>발송시간</div>
    ...
    <script language="JavaScript">
        ...
        function byte_check(cont, bytes)
        {
            ...
            //bytes.innerHTML = cnt + ' / 2,000 bytes';
            bytes.innerHTML = number_format(String(cnt)) + ' / 2,000 bytes';

            if (cnt > 2000) {
                exceed = cnt - 2000;
                alert('메시지 내용은 2,000바이트를 넘을수 없습니다.\r\n작성하신 메세지 내용은 '+ exceed +'byte가 초과되었습니다.\r\n초과된 부분은 자동으로 삭제됩니다.');
                var tcnt = 0;
                var xcnt = 0;
                var tmp = cont.value;
                ...
                //bytes.innerHTML = xcnt + ' / 2,000 bytes';
                bytes.innerHTML = number_format(String(xcnt)) + ' / 2,000 bytes';
                return;
            }
        }
    </script>
