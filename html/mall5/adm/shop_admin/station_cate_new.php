...
$sql = sql_query("
	...
	from
		shop_category_tmp a
		left join
		".$g4['yc4_category_table']."_tmp b on a.ca_id = b.ca_id
	".$where."
");
while($cate = sql_fetch_array($sql)){
	...
	$list_tr .= "
		<tr>
			...
			<td align='center'>".icon('수정',$g4['shop_admin_path'].'/station_cate_write_new.php?s_id='.$_GET['s_id'].'&uid='.$cate['uid'])."</td>
		</tr>
	";
}
...
<table width='100%'>
	<tr class='ht' align='center'>
		...
		<td>상품 갯수</td>
		<td >순서</td>
		<td><?=icon('입력',$g4['shop_admin_path'].'/station_cate_write_new.php?s_id='.$_GET['s_id']);?></td>
	</tr>
	<?=$list_tr;?>
</table>
