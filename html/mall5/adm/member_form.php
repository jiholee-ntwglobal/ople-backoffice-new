...
//sql_query("alter table {$g4['member_table']} add mb_jumin2 varchar(13) not null", false);
...
<? // 200811 : 주민번호 처리. // 김선용 2013.04 : 주민번호 미사용  ?>
<tr class=ht>
	<!--
	<td>주민번호</td>
	<td><?=substr($mb['mb_jumin2'],0,6)?>-<?=substr($mb['mb_jumin2'],6)?></td>
	-->
    <td>KCP-가상계좌 고유키</td>
    <td><? echo $mb['mb_kcp_vcnt_code']?></td>
    <td>KCP-회원고유 가상계좌</td>
    <td><? echo $mb['mb_kcp_vcnt_account']?></td>
</tr>
...
<tr class='ht'>
    ...
    <td>
        ...
        <a href="javascript:;" onclick="win_zip('fmember', 'mb_zip1', 'mb_zip2', 'mb_addr1', 'mb_addr2', 'mb_addr_jibeon','mb_zonecode');"><img src='<?=$g4[bbs_img_path]?>/btn_zip.gif' align=absmiddle border=0></a>
        ...
        <br><input type="hidden" name="mb_addr_jibeon" value="<?=$mb['mb_addr_jibeon']; ?>">
			...
			<input type="hidden" name='mb_zonecode' value="<?=$mb['mb_zonecode'];?>" />
        </td>
    <td>회원아이콘</td>
    <td colspan=3>
