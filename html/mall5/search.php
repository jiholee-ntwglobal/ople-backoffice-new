...
if($it_maker){
	...
	$sql_where .= ($sql_where ? " and ":"").$search_where." \n";
...
if(is_array($ca_arr)){
	foreach($ca_arr as $s_id => $val){
		...
		foreach($val as $row_ca_id => $val2){
			...
			if($depth == 1){
				$ca_navi_data[$s_id][$row_ca_id]++;
			}elseif($depth == 2){
				$ca_navi_data[$s_id][substr($row_ca_id,0,2)][$row_ca_id]++;
			}elseif($depth == 3){
				$ca_navi_data[$s_id][substr($row_ca_id,0,2)][substr($row_ca_id,0,4)][$row_ca_id]++;
			}elseif($depth == 4){
				$ca_navi_data[$s_id][substr($row_ca_id,0,2)][substr($row_ca_id,0,4)][substr($row_ca_id,0,6)][$row_ca_id]++;
			}elseif($depth == 5){
				$ca_navi_data[$s_id][substr($row_ca_id,0,2)][substr($row_ca_id,0,4)][substr($row_ca_id,0,6)][substr($row_ca_id,0,8)][$row_ca_id]++;
			}
			$s_cnt[$s_id]++;
		}
...
if(is_array($ca_navi_data)){
	foreach($ca_navi_data as $s_id => $val){
		...
		";
		$ca_navi .= "
				<td><input type='checkbox' name='sh_s_id[]' value='".$s_id."' ".(array_search($s_id,$_GET['sh_s_id']) !== false && $_GET['sh_s_id'] ? "checked":"")."/>".$s_name."(".number_format($s_cnt[$s_id]).")</td>
		";
		$ca_navi .= "
				<td s_id='".$s_id."'>";
		...
		if(is_array($val)){
			foreach($val as $ca_id1 => $val1){
				...
				$ca_name1 = "<span class='search_cate search_cate_1depth' ca_id='".$ca_id1."'><input type='checkbox' name='ca_id[]' value='".$ca_id1."' s_id='".$s_id."' ".(array_search($ca_id1,$_GET['ca_id']) !== false && $_GET['ca_id'] ? "checked":"")."/>".$ca_name1['ca_name']."(###_ca_cnt=".$ca_id1."_###)</span>";
				$ca_navi .= $ca_name1;
				if(is_array($val1)){
					foreach($val1 as $ca_id2 => $val2){
						$ca_name2 = sql_fetch("select ca_name from ".$g4['yc4_category_table']." where ca_id = '".$ca_id2."'");
						$ca_name2 = "<span class='search_cate search_cate_2depth' ca_id='".$ca_id2."'><input type='checkbox' name='ca_id[]' s_id='".$s_id."'  ".(array_search($ca_id2,$_GET['ca_id']) !== false && $_GET['ca_id'] ? "checked":"")."value='".$ca_id2."'/>".$ca_name2['ca_name']."(###_ca_cnt=".$ca_id2."_###)</span>";
						$ca_navi .= $ca_name2;
						if(is_array($val2)){
							foreach($val2 as $ca_id3 => $val3){
								$ca_name3 = sql_fetch("select ca_name from ".$g4['yc4_category_table']." where ca_id = '".$ca_id3."'");
								$ca_name3 = "<span class='search_cate search_cate_3depth' ca_id='".$ca_id3."'><input type='checkbox' name='ca_id[]' s_id='".$s_id."' ".(array_search($ca_id3,$_GET['ca_id']) !== false && $_GET['ca_id'] ? "checked":"")." value='".$ca_id3."'/>".$ca_name3['ca_name']."(###_ca_cnt=".$ca_id3."_###)</span>";
								$ca_navi .= $ca_name3;
								if(is_array($val3)){
									foreach($val3 as $ca_id4 => $val4){
										$ca_name4 = sql_fetch("select ca_name from ".$g4['yc4_category_table']." where ca_id = '".$ca_id4."'");
										$ca_name4 = "<span class='search_cate search_cate_4depth' ca_id='".$ca_id4."'><input type='checkbox' name='ca_id[]' s_id='".$s_id."' ".(array_search($ca_id4,$_GET['ca_id']) !== false && $_GET['ca_id'] ? "checked":"")." value='".$ca_id4."'/>".$ca_name4['ca_name']."(###_ca_cnt=".$ca_id4."_###)</span>";
										$ca_navi .= $ca_name4;
										if(is_array($val4)){
											foreach($val4 as $ca_id5 => $val5){
												$ca_name5 = sql_fetch("select ca_name from ".$g4['yc4_category_table']." where ca_id = '".$ca_id5."'");
												$ca_name5 = "<span class='search_cate search_cate_5depth' ca_id='".$ca_id5."'><input type='checkbox' name='ca_id[]' s_id='".$s_id."' ".(array_search($ca_id5,$_GET['ca_id']) !== false && $_GET['ca_id'] ? "checked":"")." value='".$ca_id5."'/>".$ca_name5['ca_name']."(###_ca_cnt=".$ca_id5."_###)</span>";
												$ca_navi .= $ca_name5;
												if(is_array($val5)){
												# 현재 4depth 까지 있음
												}else{
...
$('input[name=ca_id\\[\\]]').click(function(){
	...
	if($('input[name=ca_id\\[\\]][s_id='+s_id+']').length == $('input[name=ca_id\\[\\]][s_id='+s_id+']:checked').length){
...
$cnt_sql = "select COUNT(*) as cnt from $sql_from where $sql_where $sql_where_search ";
...
if(is_array($ca_navi)){
	...
	foreach($ca_navi as $s_id => $ca_info){
		$ca_navi_view .= "
			<tr>
				<th><input type='checkbox' name='s_id[]' value='".$s_id."'/>".$ca_navi_st_name[$s_id]."(".number_format($ca_navi_cnt[$s_id]).")</th>
				<td>
		";
		if(is_array($ca_info)){
