...
if($_SERVER['REMOTE_ADDR'] == '59.17.43.129'){
	$sql = sql_query("select is_id from yc4_item_ps_main where img_link != ''");
	$is_id_in = '';
	while($row = sql_fetch_array($sql)){
		$is_id_in .= ($is_id_in ? ",":"")."'".$row['is_id']."'";
	}

	if($is_id_in){
		$sort1 = " if(is_id in (".$is_id_in."),0,1) asc, ".$sort1;
	}
}
