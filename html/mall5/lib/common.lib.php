...
// 현재페이지, 총페이지수, 한페이지에 보여줄 행, URL (스타일 변경) - 강소진 -
function get_paging_boot($write_pages, $cur_page, $total_page, $url, $add="")
{
    $str = "";
    if ($cur_page > 1) {
        $str .= " <li class=\"page-item\"><a class=\"page-link\" href='" . $url . "1{$add}'>처음</a></li>";
    }
    ...
    if ($start_page > 1) $str .= " <li class=\"page-item\"><a class=\"page-link\" href='" . $url . ($start_page-1) . "{$add}'>이전</a></li>";

    if ($total_page > 1) {
        for ($k=$start_page;$k<=$end_page;$k++) {
            if ($cur_page != $k)
                $str .= " <li class=\"page-item\"><a class=\"page-link\" href='$url$k{$add}'><span>$k</span></a></li>";
            else
                $str .= " <li class=\"page-item active\"><a class=\"page-link\"><b>$k</b></a></li> ";
        }
    }

    if ($total_page > $end_page) $str .= " <li class=\"page-item\"><a class=\"page-link\" href='" . $url . ($end_page+1) . "{$add}'>다음</a></li>";

    if ($cur_page < $total_page) {
        //$str .= "[<a href='$url" . ($cur_page+1) . "'>다음</a>]";
        $str .= " <li class=\"page-item\"><a class=\"page-link\" href='$url$total_page{$add}'>맨끝</a></li>";
    }
    $str .= "";
...
function set_http($url)
{
    ...
    if (!preg_match("/^(http|https|ftp|telnet|news|mms)\:\/\//", $url))
        $url = "http://" . $url;
...
function get_list2($write_row, $board, $skin_path, $subject_len=40)
{
    ...
    // 분류명 링크
    $list['ca_name_href'] = "$g4[bbs_path]/board2.php?bo_table=$board[bo_table]&sca=".urlencode($list['ca_name']);

    $list['href'] = "$g4[bbs_path]/board2.php?bo_table=$board[bo_table]&wr_id=$list[wr_id]" . $qstr;
    //$list['href'] = "$g4[bbs_path]/board.php?bo_table=$board[bo_table]&wr_id=$list[wr_id]";
    if ($board['bo_use_comment'])
        $list['comment_href'] = "javascript:win_comment('$g4[bbs_path]/board2.php?bo_table=$board[bo_table]&wr_id=$list[wr_id]&cwin=1');";
    else
        $list['comment_href'] = $list['href'];
...
{
    return get_list2($write_row, $board, $skin_path, $subject_len);
}
...
function search_font($stx, $str)
{
    ...
    $replace = "<span style='color:#FA5D00;text-decoration:underline'>\\1</span>";
...
function is_admin($mb_id)
{
    ...
    if ($config['cf_admin'] == $mb_id || in_array($mb_id,array('dev','ryan13','mj45','twism'))) return 'super';
	if ($group['gr_admin'] == $mb_id || in_array($mb_id,array('ople_mrs'))) return 'group';
...
function insert_point($mb_id, $point, $content='', $rel_table='', $rel_id='', $rel_action='')
{
    ...
    # 포인트 중복적립 체크 #
    $point_chkQ = mysql_fetch_assoc(mysql_query("
		select * from g4_point where mb_id = '".$mb_id."' and po_point = '".$point."' and po_content = '".$content."' order by po_datetime desc
	"));

    $year = date('Y',strtotime($point_chkQ['po_datetime']));
    $month = date('m',strtotime($point_chkQ['po_datetime']));
    $day = date('d',strtotime($point_chkQ['po_datetime']));
    $hour = date('H',strtotime($point_chkQ['po_datetime']));
    $min = date('i',strtotime($point_chkQ['po_datetime']));
    $sec = date('s',strtotime($point_chkQ['po_datetime']));
    $gap = time() - mktime($hour,$min,$sec,$month,$day,$year);

    if($gap<=5){ // 5초 이내 동일한 포인트 적립이 있다면 적립 안함 2014-06-02 홍민기
        return false;
    }
    ...
    // 김선용 200804 : 포인트 테이블 부하방지를 위한 포인트 카운트 처리
...
function view_file_link($file, $width, $height, $content="")
{
    ...
    if (preg_match("/\.($config[cf_image_extension])$/i", $file))
        ...
        return "<img src='https://uvaxnqcpaepy770580.gcdn.ntruss.com/mall5/data/file/$board[bo_table]/".urlencode($file)."' name='target_resize_image[]' onclick='image_window(this);' style='cursor:pointer;' title='$content'>";
        ...
        return "<script>doc_write(flash_movie('https://uvaxnqcpaepy770580.gcdn.ntruss.com/mall5/data/file/$board[bo_table]/$file', '_g4_{$ids}', '$width', '$height', 'transparent'));</script>";
    //=============================================================================================
...
function mysql_pconnect($host, $user, $pass, $pconnect)
{
...
{
    global $query_link;
    return @mysqli_query($query_link, $sql);
...
{
    global $query_link;
    return @mysqli_error($query_link);
...
{
    global $query_link;
    return @mysqli_real_escape_string($query_link, $str);
...
{
    global $query_link;
    mysqli_errno($query_link);
}

function mysql_insert_id()
{
    global $query_link;
    return mysqli_insert_id($query_link);
}
// DB 연결
function sql_connect($host, $user, $pass)
{
    global $query_link;
    // 김선용 201204 : mysql 접속이 제한시점에 도달한경우 오류 방지 (new_link_flag = true)
    //return @mysql_connect($host, $user, $pass);
    $query_link = @mysql_pconnect($host, $user, $pass, true);
    return $query_link;
}
...
function mysql_escape_string($str){
    global $query_link;
    return mysqli_escape_string($query_link,$str);
}
...
function mysql_result($result, $rowno=0, $colno=0)
{
...
function sql_set_charset($charset){
    global $query_link;
    return mysqli_set_charset ($query_link,$charset);
}
...
// 김선용 201108 :
// mysql 쿼리시 인젝션 방어 및 쿼리 보안강화
function sql_safe_query($value)
{
    // 서버설정에서 get_magic_quotes_gpc 가 on 이라면 백슬래시 해제
    if(get_magic_quotes_gpc())
        $value = stripslashes($value);

    // 값이 숫자형이 아니라면 이스케이프 처리(mysql query 질의시 인젝션방어). get_magic_quotes_gpc 와는 별도로 처리됨
    if(!is_numeric($value))
...
if(!function_exists('session_unregister')){
    function session_unregister($session_nm){
        unset($_SESSION[$session_nm]);
    }
}
