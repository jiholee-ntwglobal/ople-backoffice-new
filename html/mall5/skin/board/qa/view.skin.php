...
<tr><td height=30 style="padding:5px 0 5px 0;">
	...
	<?php
	# 상품 정보 로드 #
	if($bo_table == 'iqa' && $sca == '상품문의' && $view['wr_1']){
		$it = sql_fetch("select it_id,it_name from ".$g4['yc4_item_table']." where it_id = '".trim($view['wr_1'])."'");
		if($it){
	?>

		<tr>
			<td colspan='2' style='word-break:break-all; height:28px;'>
			<?php
				echo get_it_image($it['it_id'].'_s',80,80,$it[
			'it_id'],"style='vertical-align: middle; display:inline-block; margin-right:10px;'");
				echo get_item_name($it['it_name']);
			?>
			</td>
		</tr>
		<tr><td colspan="2" height=3 style="background:url(<?=$board_skin_path?>/img/title_bg.gif) repeat-x;"></td></tr>
