...
?>
	<div class="commentviewtable" style="margin-left: <?echo strlen($list[$i][wr_comment_reply])*20;?>px;">
		<div class="commentview_top">
			<div id="left">
			...
			<? if ($list[$i][is_reply]) { echo "<a href=\"javascript:comment_box('{$comment_id}', 'c');\"><img src='$board_skin_path/img/co_btn_reply.gif' alt='답변' /></a> "; } ?>
			<? if ($list[$i][is_edit]) { echo "<a href=\"javascript:comment_box('{$comment_id}', 'cu');\"><img src='$board_skin_path/img/co_btn_modify.gif' alt='수정' /></a> "; } ?>
			<? if ($list[$i][is_del]) { echo "<a href=\"javascript:comment_delete('{$list[$i][del_link]}');\"><img src='$board_skin_path/img/co_btn_delete.gif' alt='삭제' /></a> "; } ?>
			</div>
			<div class="clear"></div>
		</div>
...
<? if ($is_comment_write) { ?>
<!-- 코멘트 입력 -->
<div id="comment_write" class="commentview_bt" style="display:none;">
	<form name="fviewcomment" method="post" action="./write_comment_update.php" onsubmit="return fviewcomment_submit(this);" autocomplete="off">
        ...
        <div class="top">
			<div class="title">
        		...
        		<? if ($is_guest) { ?>
            	이름 <INPUT type=text maxLength=20 size=10 name="wr_name" itemname="이름" required class="ed textarea_mem">
            	패스워드 <INPUT type=password maxLength=20 size=10 name="wr_password" itemname="패스워드" required class="ed textarea_mem">
            		<? if ($is_guest) { ?>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    ...
    <td width="150" class="TR"><input class="btn" type="submit" value="댓글쓰기" accesskey="s" /></td>
  </tr>
</table>
</form>
</div>
