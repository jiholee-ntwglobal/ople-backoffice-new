...
$bbs_auth_chk = sql_fetch("select count(*) as cnt from g4_auth where au_menu = '300100' and mb_id = '".$member['mb_id']."'");
$bbs_auth_chk2 = sql_fetch("select count(*) as cnt from g4_auth_new where au_menu = '300100' and mb_id = '".$member['mb_id']."'");
$bbs_auth_chk['cnt'] += $bbs_auth_chk2['cnt'];
if($bbs_auth_chk['cnt'] > 0){
    $is_admin = true;
}
...
if ($w == "")
{
    ...
    if ($member[mb_point] + $board[bo_write_point] < 0 && (!$is_admin || $bbs_auth_chk['cnt']<1 ))
...
else if ($w == "u")
{
    ...
    $sql = " select count(*) as cnt from $write_table
              where wr_parent = '$wr_id'
                ...
                and wr_is_comment = 1 ";
    $row = sql_fetch($sql);
    if ($row[cnt] >= $board[bo_count_modify] && (!$is_admin || $bbs_auth_chk['cnt']<1))
...
else if ($w == "r")
{
    ...
    if (strstr($write[wr_option], "secret")) {
        if ($write[mb_id]) {
            ...
            if (!($write[mb_id] == $member[mb_id] || $is_admin || $bbs_auth_chk['cnt']>0))
                alert("비밀글에는 자신 또는 관리자만 답변이 가능합니다.");
        } else {
            // 비회원의 경우는 비밀글에 답변이 불가함
            if (!$is_admin){
				if($bbs_auth_chk['cnt']<1){
	                alert("비회원의 비밀글에는 답변이 불가합니다.");
				}
			}
        }
    }
    if ($bo_table == 'lowset_price' && $member['mb_id']) {
        $wr_2 = $write['wr_2'];
        $wr_3 = $write['wr_3'];
        $wr_link1 = $write['wr_link1'];
...
if ($group[gr_use_access])
{
    ...
    if ($is_admin == 'super' || $group[gr_admin] == $member[mb_id] || $board[bo_admin] == $member[mb_id] || $bbs_auth_chk['cnt']>0)
        ; // 통과
    else {
        // 그룹접근
