...
if ($w == '' || $w == 'u')
{
    ...
    // 이름은 한글만 가능
    if (!check_string($mb_name, _G4_HANGUL_ + _G4_ALPHABETIC_ + _G4_SPACE_))
        alert("이름은 한글, 영문, 공백 문자만 입력 가능합니다.");
...
if ($w == "")
{
    ...
    $sql = " insert into $g4[member_table]
                set mb_id = '$mb_id',
                    ...
                    mb_9 = '$mb_9',
                    mb_10 = '$mb_10',
					mb_jumin2	= '{$_POST['mb_jumin2']}' /* 김선용 200811 : 주민번호 별도저장 */ ";
	...
	//if($_SERVER['REMOTE_ADDR'] == '59.17.43.129'){
	 if(date('Ymd') <= '20150430'){
		...
		/* 추천인 이벤트 로직 추가 2015.03.19 이성용 */

		if($mb_recommend){
			
			$sql = " select count(*) as cnt from $g4[member_table] where mb_id = '$mb_recommend' ";
			$row = sql_fetch($sql);
			if ($row[cnt] > 0){
				insert_point($mb_id, '1000', "추천인 이벤트 {$mb_recommend} 추천", '@member', $mb_id, '추천인 이벤트');
			}
		}
	}
    ...
    if($email_send_fg) {
        if ($config[cf_email_mb_member]) {
            ...
            mailer($default['de_admin_company_name'], $default['de_post_mail_addr'], $mb_email, $subject, $content, 1);
        }
        ...
        if ($config[cf_email_mb_super_admin]) {
            ...
            mailer($mb_nick, $mb_email, $default['de_receive_mail_addr'], $subject, $content, 1);
        }
    }
