...
if(($bo_table == 'qa' ||$bo_table == 'lowset_price' ) && basename(__FILE__) == 'board.php'){
	goto_url('board2.php?'.$_SERVER['QUERY_STRING']);
	exit;
}
...
if ($wr_id)
{
    ...
    // 자신의 글이거나 관리자라면 통과
    if (($write[mb_id] && $write[mb_id] == $member[mb_id]) || $is_admin || $bbs_auth_chk['cnt']>0)
        ;
    else
    {
        // 비밀글이라면
