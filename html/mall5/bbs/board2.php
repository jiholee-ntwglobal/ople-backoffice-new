...
if($bo_table == 'qa' && basename(__FILE__) == 'board.php'){
	goto_url('borad2.php?'.$_SERVER['QUERY_STRING']);
	exit;
}
...
$bbs_auth_chk = sql_fetch("select count(*) as cnt from g4_auth_new where au_menu = '300100' and mb_id = '".$member['mb_id']."'");
if($bbs_auth_chk['cnt'] == 0){
    $bbs_auth_chk = sql_fetch("select count(*) as cnt from g4_auth where au_menu = '300100' and mb_id = '".$member['mb_id']."'");
}
if($bbs_auth_chk['cnt'] > 0 && !$is_admin){
    $is_admin = true;
}
...
if ($wr_id)
{
    ...
    if ($group[gr_use_access])
    {
        ...
        // 그룹관리자 이상이라면 통과
        if ($is_admin == "super" || $is_admin == "group" || $bbs_auth_chk['cnt']>0){
	...
	if($bo_table == 'qa'){
		if($_SESSION['login_email']){
			if($write['wr_email'] == $_SESSION['login_email'] && $write['wr_password'] == sql_password($_SESSION['wr_password'])){
				$write_pass = true;
			}
		}
	}
    ...
    // 자신의 글이거나 관리자라면 통과
    if (($write[mb_id] && $write[mb_id] == $member[mb_id]) || $is_admin || $bbs_auth_chk['cnt'] > 0 || $write_pass)
        ;
    else
    {
        // 비밀글이라면
