				...
				<?php foreach ($list_data_result->result_array() as $value){ /*if($_SERVER['REMOTE_ADDR']=='211.214.213.101'){echo "<pre>"; var_dump($value); echo "</pre>";}*/ ?>
					<tr>
						...
						<td>
							...
							<a href="#" onclick="loadHistoryWorker('<?=element('channel_item_code',$value,'');?>')">
...
//		dialog.getModalHeader().hide();
	...
	// 옵션상품 변경내역 : @option_history_20200108
	function loadHistoryWorker(item_code) {
		var loadUrl = "<?=site_url('item/item_option_manage/history_worker'); ?>/" + item_code;
	    $("#file-form-content").empty().load(loadUrl);
		$(".modal-dialog").css("width", "500");
	    $("#file-form-modal").modal("show");
    }

	// 닫으면 사이즈 원복 @option_history_20200108
	$('#file-form-modal').on('hidden.bs.modal', function () {
