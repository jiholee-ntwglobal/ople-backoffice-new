...
//        $this->openmarket_db->where("sc.ca_id = 3");
		$this->openmarket_db->limit('1');
		$query	= $this->openmarket_db->get();


		echo $this->openmarket_db->last_query();

		return $query->row_array();
	}
	
	public function getMasterItemDescription($virtual_item_id){
		$this->ople_db->select('it_maker_eng, it_name_eng, desc_eng, it_explan, desc_direction, desc_warning, desc_kor');
...
# ====================================================================
    # DESC		: 오플 문구 내용 추가
    # INPUT		: string	$it_id		상품 번호
    # OUTPUT	: string	$result		오플 문구
    # --------------------------------------------------------------------
    # COMMENTS
    # --------------------------------------------------------------------
    # 2020-01-21		First
    # ====================================================================
    function getAddDescription($it_id)
    {
        $this->ople_db->select('comment');
        $this->ople_db->from('comment_insert');
        $this->ople_db->where('it_id', $it_id);
        $query = $this->ople_db->get();
        $result = $query->row_array();

        return isset($result['comment']) ? $result['comment'] : '';
    }
