...
class Single_item extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        // 매핑 수정/삭제 노출 작업자 체크
        $this->mapping_worker_ids = $this->config->item('oms_mapping_ids');
    ...
    private function list()
    {
        ...
        $data['need_update'] = element('need_update', $_GET, '');
        ...
        if($data['need_update']) $filter['need_update'] = $data['need_update'];
       ...
       $filter['master_id'] = $this->session->userdata("oms_master_id");
        ...
        //에러 상품 갯수 구하기 - KSJ 20200207
        $error_filter['need_update'] = "E";
        $error_filter['single_item'] = "single_item";
        $error_filter['master_id'] = $this->session->userdata("oms_master_id");

        $data['error_total_count'] = $this->channel_item_info_model->getChannelItemInfosCount($error_filter, "count(distinct(i.channel_item_code)) cnt");
        ...
        if (count($data['list_datas']) > 0) {
            ...
            $item_history_result = $this->channel_item_info_model->getChannelItemPriceHistory("h.*", array('item_info_id_in' => $item_info_ids), array('h.item_info_id' => "desc", 'h.create_date' => "asc"));
        ...
        if($this->input->get('excel') == 'Y') {
            ...
            if($data['need_update']=="E") {
                $this->download_Excel('뉴베이_에러단품_', $field_arr, $data);
            }else{
                $this->download_Excel('뉴베이_단품_', $field_arr, $data);
            }
            exit;
        }
        ...
        //매핑 수정/삭제 노출 작업자
        $data['worker_ids_fg'] = (in_array($this->session->userdata('oms_manager_user_id'), $this->mapping_worker_ids)) ? 1: 0;
    ...
    private function download_Excel($title,$field, $data){
        ...
        //에러상품 다운로드에 문구추가 20200224 KSJ
        if (element('need_update', $data) == "E") {

            foreach ($field as $field_key => $field_val) {
                $sheet->getCell($string++ . '2')->setValueExplicit($field_key, PHPExcel_Cell_DataType::TYPE_STRING);
            }
        }else{
            foreach ($field as $field_key => $field_val) {
        ...
        if (element('need_update', $data) == "E") {
                ...
                $sheet->mergeCells('A1:H1')
                    ->setCellValue('A1', "아래 에러메시지는 다음 동기화 시간에 자동 처리 됩니다.
    1.	재고정보가 없거나 품절인 물품의 경우 판매중으로 변경할 수 없습니다.
    2.	최대 판매기간 초과입니다. 판매기간을 2038년 이전으로 설정해주세요.
    3.	Network error :: No response!
    4.	“ExecuteReader requires an open and available Connection. The connection's current state is closed.”
    5.	“사용자 폭주로 주문 삭제에 실패하였습니다. 잠시 후 다시 시도하여 주십시오.”
");
        ...
        switch ($channel_id_info['channel_code']) {
            ...
            case 'G' : //fastople gmarket
                ...
                if($result===false){
                    ...
                    $alertMessage = ($this->apiErrorMessage != '')  ? $this->apiErrorMessage : "업데이트 오류";
                    return $this->output
                        ->set_content_type('application/json')
                        ->set_output(json_encode(array('result' => 'false', 'msg' => $alertMessage)));
                }
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	private function sendPrice($channel_item_code, $price)
	{
		...
		// 에러 메세저 저장
		$this->apiErrorMessage = element('Comment',$response);
...
//		$this->load->view('item/item_price_form_modal', $item_info);
	...
	public function InfoUpdateForm($item_info_id =''){
        ...
        if(!in_array($this->session->userdata('oms_manager_user_id'), $this->mapping_worker_ids))
        {
            return $this->output
                ->set_content_type('application/json')
...
//		$date_result = $this->channel_item_info_model->getItemPriceUpdateHistoryInfos($filter,'',array('upload_fg'=>'desc'));
    ...
    function saveUpdateProductPriceExcel(){
        ...
        $account_id = $this->session->userdata("oms_master_id");
        ...
        $upload_dir = '/ssd/html/oms/file/item_price_update/';
...
//		}
    ...
    public function gmarketItemListExcel($status){
        ...
        $this->api_key = $this->channel_info_model->getApikey(array("master_id"=>$this->session->userdata("oms_master_id"), "channel_code"=>"G"));
    ...
    public function autionItemListexcel(){
        ...
        $this->api_key = $this->channel_info_model->getApikey(array("master_id"=>$this->session->userdata("oms_master_id"), "channel_code"=>"A"));
