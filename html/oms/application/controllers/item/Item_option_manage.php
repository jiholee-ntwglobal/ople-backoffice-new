...
//		$data['list_data_result']	= $this->option_tmp_model->getChannelOptionItemInfos($filter, $select);
		...
		$header_data = array(
			...
			'add_stylesheet' => array('/oms/js/jquery-ui.min.css'),
			'add_script' => array('/oms/js/jquery-ui.min.js'),
		);
		$footer_data = array(
			'left_menu_on' => true
		);
        ...
        if($this->input->get("excel_fg") == "Y"){
            ...
            $this->downloadExcel($this->session->userdata('oms_master_id')."_옵션_리스트",$field_arr, $data);
    ...
    public function updateOptionDescription(){
        ...
        if(isset($option_item_descriptions) !== false){
            ...
            $option_item_descriptions_insert_data = array(
                ...
                "user_no" => $this->session->userdata('oms_worker_id')
            );
...
//		$channel_item_code	= $this->input->post('channel_item_code');
		...
		$upload_dir = '/ssd/html/oms/file/option_item/';
		...
		,	'channel_item_code'	=> $channel_item_code
		,	'update_user_no'	=> $this->session->userdata('oms_worker_id')
		,	'channel_update_fg'	=> 'N'
