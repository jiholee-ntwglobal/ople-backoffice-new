...
<div class="row">
    ...
    <div class="col-md-1"><button type="button" onclick="downloadOrderExcel()" class="btn btn-success">엑셀 다운로드</button></div>
    <div class="col-md-1"></div>
    <div class="col-md-1">
        <?php if($status == '1' || $status == '3'){ ?>
            <button type="button" onclick="cancelOrder()" class="btn btn-danger">선택 취소처리</button>
        <?php }elseif($status == '5'){?>
            <button type="button" onclick="cancelShippingOrder()" class="btn btn-danger">선택 취소처리(배송)</button>
        <?php }?>
    </div>
    <div class="col-md-1"></div>
    <div class="col-md-1"><button type="button" onclick="confirmOrder()" class="btn btn-primary">선택 확인처리</button></div>
    <div class="col-md-1"></div>
    <!-- <div class="col-md-1"><button type="button" onclick="confirmAllOrder()" class="btn btn-primary">전체 확인처리</button></div> -->
        ...
        <div class="table-responsive">
            <table class="table" style="font-size:10px;">
                ...
                foreach ($list_data_result->result_array() as $list_data){
                    ...
                    $shipping_order_no	= element('shipping_order_code_prefix', element(element('channel_id', $list_data),$channel_arr, array()), '').str_pad(element('order_id', $list_data), 9, "0", STR_PAD_LEFT);
                    ...
                    <tr class="<?php if(element('validate_error', $list_data, 0) > 0) echo 'bg-danger'; ?>">
                        <td><input type="checkbox" class="list-checkbox error-order-checkbox" name="order_ids[]" status="<?php echo element('status', $list_data); ?>" value="<?php echo element('order_id', $list_data); ?>"></td>
...
<script type="text/javascript">
    ...
    function cancelOrder() {
        ...
        if(confirm("선택하신 " + $(".list-checkbox:checked").length + "개의 주문서를 취소처리 하시겠습니까?")){
            ...
            $.ajax({
                ...
                async: false,
                dataType: "json",
                url: "<?php echo site_url("order/order/cancelOrder"); ?>",
    ...
    function loadMappingLayer (order_id) {
        ...
        $("#mapping-layer-content").load("<?php echo site_url('order/order_test/mapping_detail'); ?>/" + order_id);
