...
class Order extends CI_Controller
{
    ...
    private function list()
    {
        ...
        $data['worker_ids_fg'] = (in_array($this->session->userdata('qten_manager_user_id'), $this->worker_ids)) ? 1: 0;
        ...
        $data['list_data_result'] = $this->order_model->getOrders(
            ...
            i.option_name, i.product_type, i.cancel_flag,m.total_amount_usd');
    ...
    private function getListFilterData()
    {
        ...
        switch ($data['search_type']){
            ...
            case 'channel_product_no':
                $filter['channel_product_no'] = trim($data['search_value']);
                break;
            case 'buyer_name':
            ...
            foreach ($field as $field_key=>$field_val) {
                switch ($field_val){
                    ...
                    case "option_name":
                        ...
                        break;
                    case "order_id" :
                        $excel_val	= element('shipping_order_code_prefix', element(element('channel_id', $list_data),$data['channel_arr'], array()), '').date("ymd",strtotime(element('order_date',$list_data))) .str_pad(dechex(element('order_id', $list_data)), 6, "0", STR_PAD_LEFT);
    ...
    public function completeShippingOrders()
    {
        ...
        if(!in_array($this->session->userdata('qten_manager_user_id'), $this->worker_ids))
        {
            return $this->output
                ->set_content_type('application/json')
                ->set_output(json_encode(array('result' => 'error', 'msg' => '처리할 수 있는 권한을 가지고 있지 않습니다. 로그인 정보를 다시 확인해주세요.')));
    ...
    public function rollbackCancelOrder(){
        ...
        foreach ($product_order_ids as $product_order_id) {
            ...
            $order_info = $this->order_model->getOrder(array("order_item_id" => $product_order_id, "join_item"=>true,"join_channel_info"=>true), "o.*, i.*, c.channel_code");
                ...
                case '5': //배송
                    ...
                    if(element('stock_flag',$order_info)==1){
                        ...
                        foreach ($mapping_result->result_array() as $mapping_data) {
                            ...
                            $history_data = array(
                                'channel' => "qten".strtolower(element('channel_code', $order_info)),
                                'upc' => (string)$upc,
                                'sales_qty' => $upc_qty,
                                'ntics_qty' => element('currentqty', $master_item_info),
                                'dt' => date('YmdHis'),
