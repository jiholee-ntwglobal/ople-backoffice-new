...
class Oms_order extends CI_Controller
{
...
//		}
		...
		foreach($channel_arr->result_array() as $channel_info){
            ...
            switch(element('channel_code',$channel_info)){
				case 'G':
					...
					$this->getGmarketPayedOrder(element('channel_id',$channel_info),element('account_type',$channel_info));
				...
				case 'A':
					...
					$this->getAuctionPayedOrder(element('channel_id',$channel_info),element('account_type',$channel_info));
	...
	private function getGmarketPayedOrder($channel_id,$account_type)
	{
		//$this->api_key	= element('api_key',$this->sales_channel_config);
        ...
        $exchange_rate = array();
        $result = $this->order_model->getExchange_rate(array('nowdays'=>'1'),"date_format(dt,'%Y%m%d') ymd , pay");

        foreach ($result->result_array() as $exchangevalue ){
            $exchange_rate[element('ymd',$exchangevalue)] = element('pay',$exchangevalue);
            $exchange_rate['1'] = element('pay',$exchangevalue);
        }

		foreach($data as $request_order){
            echo 'GmktItemNo'.element('GmktItemNo', $request_order).PHP_EOL;
...
//			}
			...
			if(is_array(element('itemOption', $request_order, ''))) {
				...
				foreach(element('itemOption', $request_order) as $item_option){
					...
					if($v_item_id != 0) { //통관불가 추가

                        $clearance_item_info =  $this->master_item_stock_model->getVirtualMasterItemStock(array("virtual_item_id"=>$v_item_id));
                        if(!empty($clearance_item_info)) {

                            if ($account_type == 1) { //해외사업자
                                if (element('restrict_customer_clearance', $clearance_item_info) == "1") { //통관불가X
                                    $order[element('PackNo', $request_order)]['item_addition_info']['clearance_fg'] = true;
                                }
                            } else { //국내사업자
                                if (element('restrict_customer_clearance', $clearance_item_info) != "0") { //통관불가,보류X
                                    $order[element('PackNo', $request_order)]['item_addition_info']['clearance_fg'] = true;
                                }
                            }
                            echo "clean test :: " . $order_id . "//" . element('ContrNo', $request_order) . "///" . $v_item_id . "//" . $account_type . "//" . element('restrict_customer_clearance', $clearance_item_info) . PHP_EOL;
                        }
                    }
...
//					echo $order_id." :: order item insert".PHP_EOL;
			...
			// 메인상품이 단품인 경우
			    ...
			    if($channel_id=='4' || $channel_id=='5' || $channel_id=='6' || $channel_id=='7'){
                    ...
                    if(element('GmktItemNo', $request_order,'')!=''){

                        $result_vcode_cnt = $this->channel_item_info_model->getChannelItemInfosCount(array('channel_item_code'=>element('GmktItemNo', $request_order,false),'channel_id'=>$channel_id));

                        if($result_vcode_cnt>0){

                            $result_vcode_id = $this->channel_item_info_model->getItemInfo(array('channel_item_code'=>element('GmktItemNo', $request_order,false),'channel_id'=>$channel_id));

                            if(element('virtual_item_id',$result_vcode_id,'')!=''){

                                $request_order['OutItemNo']="V".str_pad(element('virtual_item_id',$result_vcode_id,''),"8","0",STR_PAD_LEFT);;
                ...
                }else if(element('OutItemNo', $request_order, '') == $v_item_id || element('OutItemNo', $request_order, '') == "") {
                    echo 'case3 :: ContrNo : '.element('ContrNo', $request_order).'||GmktItemNo : '.element('GmktItemNo', $request_order).'||OutItemNo : '.element('OutItemNo', $request_order, '').'||InventoryNo : '.element('InventoryNo', $request_order, '').PHP_EOL;
                }else{
                    echo 'case3 :: Channel:'.$channel_id.'|| ContrNo : '.element('ContrNo', $request_order).'||GmktItemNo : '.element('GmktItemNo', $request_order).'||OutItemNo : '.element('OutItemNo', $request_order, '').'||InventoryNo : '.element('InventoryNo', $request_order, '').PHP_EOL;
                }
...
//					echo $order_id." :: order item insert".PHP_EOL;
            ...
            $order[element('PackNo', $request_order)]['paydateContrDate']=element('ContrDate', $request_order);
		...
		foreach($order as $dlvNo => $info){
            ...
            if(element('clearance_fg',element('item_addition_info',$info), false) !== false){
                $validate_error	+= 16;
            }
            ...
            $ymdrequest_order = '';
            echo element('paydateContrDate', $info, false);
            if (element('paydateContrDate', $info, false)) {
                $ymdrequest_order = date('Ymd', strtotime(element('paydateContrDate', $info, false)));

                if (element($ymdrequest_order, $exchange_rate, false)) {
                    $info['amount']['total_amount_usd'] = round(($info['amount']['total_amount'] - $info['amount']['shipping_amount']) / element($ymdrequest_order, $exchange_rate, false), 2);
                } else {
                    $resultasd = $this->order_model->getExchange_rate(array('deaesearch' => $ymdrequest_order), "date_format(dt,'%Y%m%d') ymd , pay");

                    foreach ($resultasd->result_array() as $exchangevalue) {
                        $exchange_rate[element('ymd',$exchangevalue)] = element('pay',$exchangevalue);
                    }
                    ...
                    if (element($ymdrequest_order, $exchange_rate, false)) {
                        ...
                        $info['amount']['total_amount_usd'] = round(($info['amount']['total_amount'] - $info['amount']['shipping_amount']) / element('1', $exchange_rate, false), 2);
                    }
                }
            }

            if($info['amount']['total_amount_usd']>=150){
                $validate_error	+= 64;
            }
...
//				echo 'UPDATE :: '.element('order_id',$error_order).PHP_EOL;
	...
	private function getAuctionPayedOrder($channel_id,$account_type)
	{
...
//						$order[$dlvNo]['item'][]	= array(
						...
						if($v_item_id == 0) {
                            ...
                            if(element('virtual_item_id',$item_add_info) != 0) { //통관부호 추가

                                $clearance_item_info =  $this->master_item_stock_model->getVirtualMasterItemStock(array("virtual_item_id"=>element('virtual_item_id',$item_add_info)));
                                if(!empty($clearance_item_info)) {

                                    if ($account_type == 1) { //해외사업자
                                        if (element('restrict_customer_clearance', $clearance_item_info) == "1") { //통관불가X
                                            $order[$dlvNo]['item_addition_info']['clearance_fg'] = true;
                                        }
                                    } else { //국내사업자
                                        if (element('restrict_customer_clearance', $clearance_item_info) != "0") { //통관불가,보류X
                                            $order[$dlvNo]['item_addition_info']['clearance_fg'] = true;
                                    ...
                                    echo "clean test :: " . $order_id . "//" . element('OrderNo', $request_order) . "///" . element('virtual_item_id', $item_add_info) . "//" . $account_type . "//" . element('restrict_customer_clearance', $clearance_item_info) . PHP_EOL;
                                }
                            }
						}
						$this->order_item_model->addOrderItem($add_item_data);
                        ...
                        if(element('ItemNo', $request_order,'')!=''){
                            ...
                            if($result_vcode_cnt>0){
                                ...
                                if(element('virtual_item_id',$result_vcode_id,'')!=''){
                                    ...
                                    $request_order['ItemCode']="V".str_pad(element('virtual_item_id',$result_vcode_id,''),"8","0",STR_PAD_LEFT);;
...
//					$order[$dlvNo]['item'][]	= array(
                    ...
                    if($v_item_id != 0) { //통관불가 추가
                        ...
                        if(!empty($clearance_item_info)) {
                            ...
                            echo "clean test :: " . $order_id . "//" . element('OrderNo', $request_order) . "///" . $v_item_id . "//" . $account_type . "//" . element('restrict_customer_clearance', $clearance_item_info) . PHP_EOL;
                        }
                    }
...
//						echo $order_id." :: order item insert".PHP_EOL;
                ...
                $order[$dlvNo]['paydateContrDate']=element('ReceiptDate', $request_order);
			}
		}
		...
		foreach($order as $dlvNo => $info){
            ...
            if (element('paydateContrDate', $info, false)) {
                ...
                if (element($ymdrequest_order, $exchange_rate, false)) {
                    $info['amount']['total_amount_usd'] = round(($info['amount']['total_amount'] - $info['amount']['coupon_amount'] - $info['amount']['shipping_amount']) / element($ymdrequest_order, $exchange_rate, false), 2);
                } else {
                    $resultasd = $this->order_model->getExchange_rate(array('deaesearch' => $ymdrequest_order), "date_format(dt,'%Y%m%d') ymd , pay");
                    ...
                    if (element($ymdrequest_order, $exchange_rate, false)) {
                        ...
                        $info['amount']['total_amount_usd'] = round(($info['amount']['total_amount'] - $info['amount']['coupon_amount'] - $info['amount']['shipping_amount']) / element('1', $exchange_rate, false), 2);
                    }
                }
            }

            echo "달러:".$info['amount']['total_amount_usd'].PHP_EOL;
            if($info['amount']['total_amount_usd']>=150){
                $validate_error	+= 64;
            }
